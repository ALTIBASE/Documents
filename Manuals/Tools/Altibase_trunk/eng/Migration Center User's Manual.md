...
| Name                                         | Description                                                  |
| -------------------------------------------- | ------------------------------------------------------------ |
| Execution Thread                             | Specifies the maximum number of multi-thread to be used when executing data migration. The default value is triple of the number of logical processors in the server running the Migration Center. The suggested range of this value is 1 to triple of the number of logical processors in the server running the Migration Center. |
...
| **Object Options**                           |                                                              |
| Foreign Key Migration                        | Specifies whether or not to include foreign key constraints in migration target. This option is disabled by default. |
| PSM Migration                                | Specifies whether or not to include  PSM objects such as procedures, functions, materialized views, views, typesets, and triggers in migration target. This option is enabled by default.<br /> |
| Drop Existing Objects                        | Specifies whether or not to recreate the database object before data migration. <br />When this option is enabled, DROP and CREATE will be executed on every objects in target database. When this option is disabled, data migration will be executed without dropping the database objects. This option is disabled by default. |
| Keep Partition Table                         | Specifies whether or not to maintain partitioned tables.<br />When this option is enabled, partitioned tables will be created the same as the source database. When this option is disabled, it will be created as a non-partitioned table. This option is disabled by default. |
| Use Double-quoted Identifier                 | Specifies whether or not to use double quotation marks for schema and object names. This option is disabled by default. |
| Remove FORCE from View DDL                   | Specifies whether or not to remove 'FORCE' keyword from the statement creating a view. |
| Postfix for reserved word                    | Specifies a user-defined word which is to be added to the database object name in the source database as a postfix when it conflicts with a reserved keyword of Altibase. |
| **Data Options**                             |                                                              |
| Batch Execution                              | Specifies whether or not to use batch insert in JDBC for higher performance. This option is enabled by default. |
| Batch Size                                   | Specifies the batch size when batch insert in JDBC is used. Default value is 10,000. |
| Batch LOB type                               | Specifies whether or not to batch process BLOB and CLOB data types.<br />When this option is enabled, Out Of Memory (OOM) error can occur according to the size of the data, and exceptions can occur in some databases that do not support this feature, such as TimesTen. This option is disabled by default. |
| Log Insert-failed Data                       | Specifies whether or not to log rows that failed inserting data during DB to DB data migration. This option is available only when the Batch Execution option is disabled. This option is disabled by default. |
| File Encoding                                | Specifies the encoding character set to be used when recording the records that failed inseting data to files. This option is available only when the Log Insert-failed Data option is enabled. Default value is UTF8. |
...
| Data Sampling                                | Specifies whether or not to use data sample feature.<br />When this option is enabled, to decrease the data validation time, only sampling data will be validated. When this option is disabled, every data will be validated. This option is enabled by default. |
| Percent Sampling (exact counting)            | Specifies the percentage of data to be sampled from the table. This option is used only when the Exact Counting Method is selected in the build stage. |
| Record Count Sampling (approximate counting) | Specifies the number of records to be sampled from the table. This option is used only if the Approximate Counting Method is selected in the build stage. |
...
| Name                                                  | Description                                                  |
| ----------------------------------------------------- | ------------------------------------------------------------ |
| Execution Thread                                      | Specifies the maximum number of multi thread to be used when executing data migration. Default value is triple of the amount logical processors in the server running the Migration Center. The suggested range of this value is 1 to triple of the amount logical processors in the server running the Migration Center. |
| Migration Target                                      | Specifies the targets for data migration: <br />\- Object & Data: Database objects and table data<br />\- Object: Database objects only |
| **Object Options**                                    |                                                              |
| Foreign Key Migration                                 | Specifies whether or not to include foreign key constraints in migration target. This option is disabled by default.. |
| PSM Migration                                         | Specifies whether or not to include  PSM objects such as procedures, functions, materialized views, views, typesets, and triggers. This option is enabled by default..<br /> |
| Keep Partition Table                                  | Specifies whether or not to maintain partitioned tables.<br />When this option is enabled', partitioned tables will be created as same as the source database. When this option is disabled, it will be created as non-partitioned table. This option is disabled by default. |
| Use Double-quoted IdentifierData Files: File Encoding | Specifies whether or not to use double quotation marks for schema and object names. This option is disabled by default.Specifies the encoding character set to be used for output scripts and data files. |
| **Data Files**                                        |                                                              |
| File Encoding                                         | Specifies the encoding character set to be used for outputs of scripts and data file. |
