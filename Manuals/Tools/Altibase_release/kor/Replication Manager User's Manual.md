...
이 절은 Altibase에 연결하기 위한 기본적인 단계를 길라잡이 형태로 설명한다.
Altibase와 함께 Replication Manager를 실행할 때의 작업 흐름은 보통 다음과 같다.
...
Altibase를 설치하고 앞선 두 가지 작업을 마침으로써 데이터베이스에 연결할 준비가
완료되었다.
