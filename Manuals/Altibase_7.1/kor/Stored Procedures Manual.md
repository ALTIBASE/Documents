...
사용자가 명시적으로 저장 프로시저를 컴파일 때 사용된다.
...
무효한 상태의 저장 프로시저가 호출되면, Altibase 서버는 자동으로 재 컴파일 한다. 그러나 런타임 시에 컴파일하는 것은 심각한 성능 이슈를 야기할 수 있으므로, 프로시저가 무효한 상태가 되면 ALTER PROCEDURE 문을 이용하여 수동으로 컴파일 하는것을 권장한다.
