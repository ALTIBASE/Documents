...
| \-d *datafile(datafiles)*                 | 데이터베이스에서 파일로, 또는 파일에서 데이터베이스로 자료를 복사할 때 자료 파일의 전체 경로를 지정한다. `formout`인 경우에는 사용되지 않는다. <br />`in` 모드:<br />- 최대 32개의 파일을 입력할 수 있으며, 입력한 순서대로 업로드한다.<br />- 파일 하나 당 정상적으로 업로드 할 수 있는 최대 건수는 2,147,483,647건이다. 최대 건수를 초과할 경우, 초과된 데이터는 무시되고 업로드 작업을 종료한다. 이때, 로그 파일의 처리 건수 기록에 오류가 발생할 수 있다.<br />`out` 모드:<br />- 파일 하나 당 정상적으로 다운로드 할 수 있는 건수는 2,147,483,647건이다. 초과된 데이터는 다운로드할 수 있으나, 로그 파일의 처리 건수 기록에 오류가 발생할 수 있다.<br />- parallel 옵션과 함께 사용하면, 최소 parallel 옵션에서 지정한 수만큼 파일을 생성한다. |
...
| -verbose                                  | 업로드 에러가 발생한 경우, 에러가 발생한 [칼럼의 위치(COLUMN_ORDER)](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_7.1/kor/General%20Reference-2.The%20Data%20Dictionary.md#sys_columns_)를 -log 옵션에서 설정한 로그 파일에 기록한다. 다만, 에러가 발생한 위치를 알 수 없을 때는 기록하지 않는다.<br>이 옵션을 사용하기 위해선 -log 옵션을 반드시 설정해야 한다. |
