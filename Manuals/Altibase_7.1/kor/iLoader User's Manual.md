...
> ```
>
> CSV 포맷 규칙을 따르면 쉼표나 쌍따옴표가 포함된 데이터도 정상적으로 처리되기 때문에 사용자가 데이터의 내용을 고려하지 않아도 된다. 하지만 **-rule csv 옵션을 사용할 경우 구분자를 임의로 지정하는  -f, -t, -r 그리고 -e 옵션은 사용할 수 없다**는 것을 유의해야 한다. 
