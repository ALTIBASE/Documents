...
| \-rule csv                                | 업로드 또는 다운로드 할 데이터 파일에 [csv 규칙](#rule-csv)을 적용한다.<br/>구분자 관련 옵션인  -f, -t, -r, 그리고 -e와 함께 사용할 수 없다. |
...
| -extra_col_delimiter                      | 레코드 마지막 컬럼 뒤에 컬럼 구분자와 레코드 구분자가 연달아 위치한 경우, 이를 레코드의 끝으로 인식하기 위한 옵션이다.<br/> 예를 들어, 컬럼 구분자가 '^'이고 레코드 구분자가 '\n'인 데이터 파일이, 아래와 같은 형식이면 -extra_col_delimiter 옵션이 필요하다.<br/>Kim^1077^RD^\n<br/>Lee^1099^CS^\n<br/>이 옵션은 -rule csv 또는 -t 옵션과 함께 사용할 수 있다. |
...
>
>  iLoader는 기본적으로 CSV(comma-separated values) 포맷 규칙에 따라 데이터를 처리한다. CSV 포맷이란 쉼표로 필드 혹은 칼럼을, 줄바꿈으로 행을 구분하는 텍스트 기반 데이터 포맷이다. CSV 포맷 규칙이 적용된 데이터는 다음과 같이 형식이 변환된다.
>
> ```
...
> ```
>
> iLoader는 쉼표나 쌍따옴표가 포함된 데이터도 정상적으로 처리할 수 있기 때문에 사용자가 데이터의 내용을 고려하지 않아도 된다. 하지만 - rule csv 옵션을 사용할 경우 데이터가 csv 포맷 규칙을 따르기 때문에, **구분자를 임의로 지정하는  -f, -t, -r 그리고 -e 옵션과 함께 사용할 수 없음**을 유의해야 한다. 
