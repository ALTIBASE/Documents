...
* OPTIONS OFFLINE *log_dir*: 이 옵션은 jdbcAdapter와 oraAdapter에서 사용하는 옵션으로, 오프라인 이중화 옵션을 설정하기 위해 사용한다. *log_dir*은 Active 서버의 로그파일 경로를 입력하며, 로그파일의 경로가 여러개일 경우 콤마(,)를 이용하여 나열한다.
* OPTIONS META_LOGGING : 이 옵션은 jdbcAdapter와 oraAdapter에서 사용하는 옵션으로, 메타 정보와 재시작 SN 정보를 파일로 저장할 때 사용한다. 이 파일은 로그파일 경로에 저장된다. 
