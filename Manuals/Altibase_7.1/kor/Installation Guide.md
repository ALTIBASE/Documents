...
> ###### 주의사항
>
> 현 사용중인 서버의 메타 버전이 8.8.1 이상이고 다운그레이드 할 메타 버전이 8.7.1 이하이면서  GEOMETRY 타입의 컬럼이 있는 테이블이 1개 이상 있는 경우 다운그레이드가 실패합니다.
> 
>데이터베이스를 재구성하거나 GEOMETRY 타입의 컬럼이 있는 테이블만 백업하고 테이블을 삭제한 후에 메타 다운그레이드를 수행해야 합니다.
