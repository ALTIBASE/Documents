...
- Altibase는 NOT NULL 제약조건이 정의된 LOB 칼럼에 데이터를 입력할 경우, LOB Locator 값을 얻어오기 위해 NULL 값으로 초기화하는 내부 특성으로 인해 제약조건 위배 에러가 발생한다. 이것은 Lob 컬럼에 대한 NOT NULL 제약 조건이 타 DB와의 마이그레이션 작업 편의성 등을 위한 Syntax 호환 제공의 기능으로서만 동작하기 때문이다.
