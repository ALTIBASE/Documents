...
-   ALTERNATE노드는 하나만 설정 가능하다. 2개 이상의 ALTERNATE노드 지원하지 않는다.

#### 데이터 제약조건

-   샤드 키 테이블에서 기본 키 또는 유니크 속성을 갖는 컬럼은 샤드 키와 동일한 컬럼이어야 한다.
...
-   여러 개의 샤드 노드들 중에서 일부 샤드 노드에 네트워크나 서버 장애가 발생하는 경우가 있다. 
    ...
    - 에러 처리는 일부 샤드 노드의 장애를 전체 데이터베이스의 장애로 판단한다.
    - 장애 노드를 접근하지 않는 작업은 정상 동작한다.
...
-   CLONE 객체에 대한 장애처리는 사용자가 직접 수행해야 한다.
...
- 한번에 하나의 PARTITION에 대하여서만 리샤딩이 가능하다.
