...
이름이 없이 독립적으로 저장 프로시저 블록을 사용할 수 있으며, 이를 익명 블록(anonymous block)라고 한다.
익명 블록은 7.1.0.2.3 버전부터 지원하며 아래와 같은 특징이 있다.

- PSM 객체를 생성 및 데이터베이스에 저장하지 않고 한 번만 사용할 수 있다.
- RETURN 절을 사용할 수 없다.
- 프로시저와 달리 IN, OUT, INOUT 바인드 변수(BIND Variable)를 사용할 수 있다.
