...
DELETE OFF 인 경우 큐에 DELETE 구문을 사용할 수 없고 DEQUEUE 병렬 수행할때 성능이 향상된다.
...
큐에 DELETE 구문을 수행할지 여부를 결정할지 정하는 절이다.
...
큐의 상태는V\$QUEUE_DELETE_OFF를 통해 확인 할 수 있다.(자세한 설명은 *General Reference*를 참고한다.)
