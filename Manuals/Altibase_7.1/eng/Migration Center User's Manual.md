...
| Unique Constraint      | O                          | O                           |                                                              |
| Foreign Key Constraint | O                          | O                           |                                                              |
| Index                  | O                          | O                           | The index ordering(ASC/DESC) or size of TimesTen(the original database) is not provided. Thus, the index ordering is set as the default value(ASC), and the size is not displayed when migrating. Among the three indexes(hash, range, bitmap) provided by Times ten, the hash•range index is converted into B-tree index of Altibase and the bitmap index is skipped when migrating. Furthermore, if there is a primary key or unique constraint on the index column, the index is excluded from migration because it is not allowed by Altibase and can be checked in the Missing tab of the build report.|
| Sequence               | O                          | X                           |                                                              |
| Private Synonym        | Partly yes                 | X                           | Only synonyms that refer to objects in the same schema are migrated. |
