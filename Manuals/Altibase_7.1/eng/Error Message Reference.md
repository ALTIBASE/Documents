...
**Action:** Verify that there are enough system resources. Check the error number from the trace log and contact Altibase's Support Center (http://support.altibase.com/en/).
...
**Action:** Check the error number from altibase_sm.log and contact Altibase's Support Center (http://support.altibase.com/en/).
...
**Action:** Check the error message and contact Altibase's Support Center (http://support.altibase.com/en/).
...
**Action:** Check the replication protocol version between the two nodes. If the versions are different, change it to the same version. If the versions are the same, check the error number from the trace log and contact Altibase’s Support Center (http://support.altibase.com/en/).
...
**Action:** Restart the replication Sender. If the replication Sender repeatedly fails to start, check the error number from the trace log and contact Altibase’s Support Center (http://support.altibase.com/en/).
...
**Cause:** LOB locator was not found.
...
**Action:** Replication recovery is impossible if the corresponding server has been terminated. To disable this option, set REPLICATION_RECOVERY_MAX_TIME to 0, and restart. If a network error has occurred, increase REPLICATION_RECOVERY_REQUEST_TIMEOUT, and restart replication. If connection fails, check the error number from the trace log and contact Altibase’s Support Center (http://support.altibase.com/en/).

**0x610B0 ( 397488) rpERR_ABORT_MISMATCH_OLD_ITEMS_COUNT Old items count mismatch (\<0%d\>:\<1%d\>)**

**Cause:** The number of rows selected from SYS_REPL_OLD_ITEMS\_ is invalid due to a database error.
...
**Cause:** Failed to update the REMOTE_FAULT_DETECT_TIME column in the SYS_REPLICATIONS\_ meta table due to a database error.
...
- Check the error number from the trace log and contact Altibase's Support Center (http://support.altibase.com/en/).
...
**0x61183 ( 397699)
rpERR_ABORT_RP_DDL_SYNC_NOT_SUPPORT_GLOBAL_NON_PARTITION_INDEX A DDL synchronization is not support on a table with global non-partitioned index.**
...
**Action:** Check the error number from the trace log for more detailed information. And contact Altibase's Support Center (http://support.altibase.com/en/).
...
**Action:** Verify the appropriate values for __OPTIMIZER_VERSION property.
...
**0x313CF ( 201679) qpERR_ABORT_QSV_INVALID_PRAGMA_RESTRICT_REFERENCE Pragma
