...
jdbcAdapter is an application program running in JRE version 7 or later; thus, JRE 7 or later versions should be installed in which jdbcAdapter will be performed. 

If the JDBC driver for the remote database to which jdbcAdapter will connect is running on  JRE version 7 or later, a version of the JRE compatible with the JDBC driver should be installed. 
...
7. Once all of the property settings pertaining to the use of jdbcAdapter, a dialog box showing specified values will appear. Then, verify if every property is properly specified,  and click "Forwawrd" to proceed to the next
...
-   JDBC_ADAPTER_HOME:  This environment variable will have jdbcAdapter home directory specified in the previous step before the installation process as a value.
...
-   bin directory  
    The executable and internally used files of dbcAdapter (Adapter for JDBC) and oaUtility (Adapter for JDBC utility) are located in this directory.
...
This property indicates retry intervals between error occurrences when applying records.
