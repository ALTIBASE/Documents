...
Array-type host variables can be used with the EXECUTE statement. When array-type host variables are used with the EXECUTE statement, executing the EXECUTE statement a single time has the same effect as executing the EXECUTE statement a number of time equal to the size of the array, thereby expecting a performance improvement.

#### Array Types

When used in the EXECUTE statement, array-type host variables can only be used as IN parameters.

The following are the array types that can be used in the EXECUTE statement:

- Numeric type, character type arrays
- Structures containing arrays as their elements
...
- Array-type host variable cannot be used to return value of a stored function.
...
##### 2 dimensional array cannot be used as a host variable.
