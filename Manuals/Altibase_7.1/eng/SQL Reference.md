...
[access_mode_clause::=](#access_mode_clause_CREATETALBE)

[physical_attributes_clause::=](#create_table_physical_attributes_clause)

[log_compression_clause::=](#log_compression_clause)

[logging_clause ::=](#logging_clause)
[parallel_clause::=](#parallel_clause)

[table_compression_clause::=](#table_compression_clause)
...
[disk_index_attributes::=](#disk_index_attributes)
...
> Note: The user can manually request materialized view refresh by calling the REFRESH_MATERIALIZED_VIEW stored procedure. For further information on the REFRESH_MATERIALIZED_VIEW stored procedure, refer to Chapter 10 in the *Stored Procedures Manual.*
...
| 242    |                   | CREATE ANY TRIGGER           | For creating triggers not only in the user's own schema but also in other user's schema |
...
| 253    |                   | CREATE ANY MATERIALIZED VIEW | This privilege can create a materialized view not only in the user's own schema, but also in other user's schema |
...
| 266    | JOB               | CREATE ANY JOB               | This privilege can create a new job not only in the user's own schema, but also in other users' schema. |
