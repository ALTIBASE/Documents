...
| <CpuSamplingInterval\>                                  | Optional  | This is the execution cycle of the thread that measures CPU utilization (%). The default value is 3 and the unit is second.<br />CPU utilization is affected by the measurement interval. When using multiple CPU monitoring elements, inaccurate values may be returned due to mutual time interference. To prevent this, a CPU utilization measurement thread independent of the monitoring element measures the value at set intervals, and the monitoring component refers to the measured value.<br />The OS CPU and Altibase CPU utilization measurement threads are separate threads. , it is possible to cause a time difference between the two values of TOTAL_CPU and PROC_CPU when the CPU is overloaded.<br />CpuSamplingInterval is provided from Altibase 7.1.0.8.4 or higher version. |
