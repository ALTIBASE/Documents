...
* A table that includes LOB data type has a constraint for the following three properties. For more information, please refer to the description of each property.
  - [ORACLE_ERROR_RETRY_COUNT](#oracle_error_retry_count-unit-number)
  - [ORACLE_SKIP_ERROR](#oracle_skip_error)
  - [ORACLE_ARRAY_DML_MAX_SIZE](#oracle_array_dml_max_size)
...
* If users do not commit, LOB data updated by SELECT FOR UPDATE may not be replicated in the following situations:
  ...
  - If SKIP occurs due to the ORACLE_SKIP_ERROR, ORACLE_SKIP_INSERT, or ORACLE_SKIP_UPDATE properties during replication of data modified prior to executing SELECT FOR UPDATE.
