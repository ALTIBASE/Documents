...
#### 알티베이스 샤딩의 업무 적용 범위
- 대량데이터에 대한, 단순 OLTP (Online Transaction Processing) 업무이고, 데이터 유실없는 HA가 보장되어야 하는 업무
- 좀더 기술적으로 표현하면, 데이터는 매우 크지만, 대부분의 쿼리가 클라이언트측 샤딩으로 분류되는 업무이고, 데이터 유실없는 HA가 보장되어야 하는 업무
