...
- sys 계정의 암호 변경
  - 샤드노드들 간에는 sys 계정으로 연동합니다. 그러므로, 모든 샤드노드의 sys 계정의 암호는 동일하어야 합니다.
  - 특정 샤드노드의 sys 계정의 암호를 변경하면, 타 샤드노드들과 연동이 되지 않아 문제가 발생합니다.
  - sys 계정의 암호 변경을 위해서는 모든 샤드노드들은 shutdown 한 후에 재구동 한 상태에서 동일하게 암호를 변경한 후에 모든 샤드노드들은 샤딩 클러스터에 JOIN 시킵니다.
