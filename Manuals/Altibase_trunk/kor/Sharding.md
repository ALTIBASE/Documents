...
-   샤드 키 테이블에서 기본 키는 샤드 키를 포함하여야 한다.
...
- 이때 샤딩관련 객체에는, 
  - 샤드테이블과 
  - 해당 샤드테이블에 대해서 자동 생성되는 _BAK_ 테이블, 샤드프로시져 그리고, 샤드테이블의 복제를 위하여 자동 생성되는 이중화객체가 있습니다.
- 샤딩관련 객체에 DDL을 수행하기 위해서는, 샤딩객체에서 설정해제하고, DDL을 한 이후에 다시 샤딩객체로 설정하여야 합니다.
- 예외적으로 아래의 DDL은 허용합니다.
차기샤딩에서는, 현재로써는, online partition split에 대해서만, 샤딩객체에서 설정해제하지 않고, 수행할수 있는 별도 기능을 만드는 것이 예약되어 있습니다.
