...
3. 첫번째로 추가될 샤드 노드에 아래 구문을 수행하여, k-safety 관련 전역 정보를 설정한다.
   - DBMS_SHARD.SET_REPLICATION(...)
   - 두번째 이후로 추가되는 노드들은 별도의 설정없이 동일한 k-safety 관련 전역 정보를 사용한다.
   - 모든 노드에 설정해도 되지만, 전역정보 이므로, 첫 번째로 추가되는 노드의 설정과 동일하여야 한다.
4. 샤드 노드별로 각종 데이타베이스 객체들을 생성한다.
   - 객체 생성 DDL을 사용한다.
   - 모든 샤드 노드들은 동일한 데이타베이스 객체들을 갖아야 한다.
5. 샤드 노드별로 아래 구문을 수행하여, 모든 노드들을 샤딩 클러스터에 추가한다.
