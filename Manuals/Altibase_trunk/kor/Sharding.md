...
#### 상황별 고려사항
- k-safety가 1 이상에서, 특정 노드를 shutdown 했는데, 해당 노드에 영구장애 발생후, 해당 노드가 불완전복구된 상황
...
- k-safety가 1 이상에서, 노드 장애로 failover되어 데이타 손실없이 정상운영되고 있는데, 장애 노드가 불완전복구된 상황
...
- k-safety가 1 이상에서, 여러 노드들이 죽어서, 정상 failover되지 못한 노드가 있는데, failback 되어야할 노드가 불완전복구된 상황
  - 이 경우에 해당 불완전복구된 노드를 failback 시키면, 해당 노드에서 서비스하던 데이터들만 손실이 발생하는 것이 아니다. 이후에, 해당 노드로 백업을 하던 노드에 장애가 발생하면, failover시 데이타 손실이 발생할 수 있다.
...
- k-safety가 0 에서, 장애노드가 불완전복구된 상황
  - 불완전복구된 것이 가장 최신의 데이타를 갖고 있는 상황이라면, 불완전복구된 상태로 failback 혹은 join 시켜서 사용한다.
  - 오프라인 백업본이 가장 최신의 데이타를 갖고 있는 상황이라면, 오프라인 복구시킨 상태로 failback 혹은 join 시켜서 사용한다.
  - failback 혹은 join 이후에, 추가로 보완할 데이타가 확보되면, 해당 데이타는 샤딩클러스터에 논리적 복구를 이용하여 데이터를 다시 적재하여 사용할 수 있다.
  - 특정 노드에 영구장애가 발생하여, 어떠한 형태로도 failback 혹은 join 시켜서 사용할수 없는 경우에는, 
    - reorganize SHARD DDL을 이용하여, 해당 노드를 샤딩클러스터에서 제거한다.
