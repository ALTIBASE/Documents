...
아래의 모든 작업은 sysdba로 작업하는것을 가정한다.
...
5. 샤드 노드별로 아래 구문을 수행하여, 모든 노드들을 샤딩 클러스터에 추가한다.
   ...
   - 개별 샤드 노드들은 샤딩 클러스터에 추가되지 전까지는, sysdba 만 접속이 가능하고, 다른 클라이언트의 접속은 차단된다.
6. 샤딩 클러스터에 참여된 하나의 노드에서, 아래 구문들을 이용하여, 객체별 분산정의를 한다.
   - DBMS_SHARD.SET_SHARD_TABLE(...), DBMS_SHARD.SET_SHARD_RANGE(...) 등등
   - 위 구문들을 이용하여, 분산정의를 한후에, COMMIT 을 수행하면, 샤딩 클러스터에 참여된 모든 샤드노드들에 동시에 적용된다.
