...
- GLOBAL_TRANSACTION_LEVEL 이 3 인 경우는 count(\*) 최적화를 적용하지 않는다.
  ...
  - count(\*) 최적화에서는 two phase commit pending 상태의 레코드들은 고려되지 않으므로, two phase commit 에서는 적합하지 않은 방식이다.
  - 특히나, SHARD_NOTIFIER_2PC 가 1 이고 GLOBAL_TRANSACTION_LEVEL 가 3 인 경우에는 two phase commit 이 지연처리가 되므로, count(\*) 최적화는 적용되면 안된다.
