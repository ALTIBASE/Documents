...
- [Altibase Sharding Guide](#altibase-sharding-guide)
  ...
  - [Altibase Sharding 설치](#altibase-sharding-%EC%84%A4%EC%B9%98)
    - [Altibase Sharding 설치](#altibase-sharding-%EC%84%A4%EC%B9%98-1)
    - [Zookeeper 설정](#zookeeper-%EC%84%A4%EC%A0%95)
  ...
  - [Altibase Sharding 사용](#altibase-sharding-%EC%82%AC%EC%9A%A9)
    ...
    - [샤드 메타 관리](#%EC%83%A4%EB%93%9C-%EB%A9%94%ED%83%80-%EA%B4%80%EB%A6%AC)
    - [샤드 메타 생성](#%EC%83%A4%EB%93%9C-%EB%A9%94%ED%83%80-%EC%83%9D%EC%84%B1)
    - [지역 사드 노드 설정](#%EC%A7%80%EC%97%AD-%EC%82%AC%EB%93%9C-%EB%85%B8%EB%93%9C-%EC%84%A4%EC%A0%95)
    - [샤드 복제 정보 설정](#%EC%83%A4%EB%93%9C-%EB%B3%B5%EC%A0%9C-%EC%A0%95%EB%B3%B4-%EC%84%A4%EC%A0%95)
    - [(TOBEMODIFIED) 샤드 노드 추가](#tobemodified-%EC%83%A4%EB%93%9C-%EB%85%B8%EB%93%9C-%EC%B6%94%EA%B0%80)
    - [샤드 노드 삭제](#%EC%83%A4%EB%93%9C-%EB%85%B8%EB%93%9C-%EC%82%AD%EC%A0%9C)
    ...
    - [(TOBEMODIFIED) 샤드 트랜잭션](#tobemodified-%EC%83%A4%EB%93%9C-%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98)
  ...
  - [Shard DDL](#shard-ddl)
    - [ALTER DATABASE SHARD ADD](#alter-database-shard-add)
  ...
  - [Altibase Sharding 딕셔너리](#altibase-sharding-%EB%94%95%EC%85%94%EB%84%88%EB%A6%AC)
    ...
    - [SYS_SHARD.REPLICA_SETS_](#sys_shardreplica_sets_)
    - [SYS_SHARD.FAILOVER_HISTORYS_](#sys_shardfailover_historys_)
  ...
  - [Aktibase Sharding 유틸리티](#aktibase-sharding-%EC%9C%A0%ED%8B%B8%EB%A6%AC%ED%8B%B0)
...
#### 샤드 클러스터 관리자 메타(Zookeeper meta)

Zookeeper에 샤드 클러스터 관리를 위하여 관리하는 정보를 의미한다.
...
- 논리적 백업/복구 고려사항
  - 샤딩클러스터에 참여된 상태에서는 기본적으로 쿼리 수행의 결과는 특정 노드에 대한 것이 아니고, 전체 클러스터에 대한 것이다.
  - 샤딩클러스터에 참여된 상태에서도 NODE[META] 샤드 키워드를 사용하면, 쿼리 수행의 결과를 사용자 세션이 접속한 샤드 노드로 국한 시킬 수 있다.
  - iLoader에서도 NODE[META] 샤드 키워드를 사용할 수 있으며, 이에 대한 내용은 iLoader 매뉴얼을 참고한다.
- 오프라인 백업/복구 고려사항
  - 오프라인백업시에 xlogfile들도 백업해야 한다.
  - 샤딩클러스터 전체 노드들에 대하여, 한꺼번에 오프라인백업을 한 경우에는, 샤딩클러스터 전체 노드에 대하여 한꺼번에 복구하는 용도로 사용할수 있다. 
  - 오프라인백업을 이용하여, 특정 노드만을 복구하는 경우에는, 해당 노드는 샤딩클러스터에 failback 혹은 join 시켜서 사용할수 없다.
- 온라인 백업/복구 고려사항
  - 완전복구된 노드에 대해서만, 샤딩클러스터에 failback 혹은 join 시켜서 사용하는 것을 기본으로 한다.



Restore
완전복구가 되지 않는 경우에는 필요한 테이블들을 논리적 백업을 한후에, 해당 노드는 없애고, 다시 노드추가 후에, 데이타를 업로드 한다.

불완전 복구후에 샤딩에 참여하는 방안
k-safety = 0 인 경우의 노드 하나가 죽었다가 restart가 안되는 경우

즉, failover 혹은, 죽은 노드가 있는 경우는, failback시에 SMN 복구가 안될만큼의, SMN이 변경되는 작업은 원천적으로 일어나지 않도록, 샤딩에서 막혀있다. 이런 작업이 필요하면, 아예 node drop 후에 작업을 하도록 되어 있고, node drop 후에는 아예 failback 이란것을 시킬수 없고, 빈 노드로 add만 된다.

셧다운후에 영구장애로 join을 못시키는 경우에 작업 방안
일단 강제 failover 시킨다. 그러면 서비스는 진행이 된다.
그후에 완전복구가 되면, failback 시킨다.
완전복구가 안되면, 아예 해당 node를 drop 시키고, 빈 노드로 add 하는 방법이 있고, failback with sync를 쓰는 방법이 있을수 있음.
백업된DB를 단독DB로 띄워서, 과거시점의 데이타만 확인하는 방안
각 노드별로 각기 다른 시점에 온라인 백업된 화일들과 각 노드별 아카이브화일들을 이용하여, 클러스터 전체를 복구하는 방안
...
-   샤드키는 테이블별로 1개만 지정할 수 있다.
-   샤드키 테이블에서 기본 키는 샤드 키를 포함하여야 한다.
-   샤드키 테이블은 분산 방식에 대응되는 파티션드 테이블(partitioned table)로 생성해야 하며 파티션 키와 샤드 키는 동일해야 한다.
...
4. 샤드 노드별로 각종 데이타베이스 객체들을 생성한다.
   - 객체 생성 DDL을 사용한다.
   - 모든 샤드 노드들은 동일한 데이타베이스 객체들을 갖아야 한다.
...
* Meta SMN
  ...
  * 샤드 메타를 변경하는 트랜잭션 내부에서만 존재하고, 해당 트랜잭션이 COMMIT 이 되는 순간 Data SMN 에 적용이 된다.
...
Meta SMN의 변경은 실제 Altibase Sharding 시스템에서 인식하지 않으며, 해당 샤드 메타 변경 트랜잭션이 COMMIT 되는 순간 각 노드의 Data SMN을 변경하여 이후의 작업들이 변경된 Data SMN을 기준으로 동작한다. 

각 세션은 최초 접속 시에 Data SMN을 자신의 Session SMN으로 할당받아 수행한다.

Data SMN이 변경되면 이전에 접속한 세션은 각각 자신의 접속 시점에 유지되던 Session SMN과 다르게 된다. 이를 보정해 주기 위한 작업은 Altibase 내부적으로 자동으로 처리된다.
...
k_safety: 시스템 내에서 유지할 복제본의 개수를 지정한다.  0, 1, 2를 지원하며 '0'으로 설정하면 복제를 하지 않는다.

(TOBEMODIFIED) replication_mode: 'CONSISTENT', 'NOWAIT' 의 두 가지 모드를 지원하며, 'CONSISTENT' 는 동기 모드로 COMMIT시 복제본에도 로그가 기록된 것을 확인하고 반환하여 일부 성능 저하가 발생할 수 있으나 복제본의 데이터 일관성을 보장한다.  'NOWAIT'은 비동기 모드로 복제 처리가 서비스 트랜잭션에 영향을 주지 않아 성능이 우수하다. 다만, 복제본의 데이터는 손실이 발생하거나 일관성이 깨질 수 있다. 
...
샤드 노드별로 아래의 SHARD DDL 구문을 수행하면, 해당 샤드 노드가 샤딩 클러스터에 추가된다.

ALTER DATABASE SHARD ADD;

개별 샤드 노드들은 샤딩 클러스터에 추가되지 전까지는, sys 사용자를 제외한 다른 일반 사용자의 접속은 차단된다.

샤딩 클러스터 전체가 모두 정상적인 상태일 경우에만, 샤드 노드 추가를 할 수 있다. 즉, 아래와 같은 경우에는 해당 이슈를 해결한 이후에 샤드 노드를 추가할 수 있다.
- JOIN 하지 않은 노드가 있다면, 먼저 해당 노드를 JOIN 시켜야 한다.
- failover 당한 노드가 있다면, failback을 하거나, 해당 failover 당한 노드를 클러스터에서 삭제해야 한다.
- 다른 세션에서 이미 SHARD DDL을 수행중이면, 해당 SHARD DDL의 수행이 완료되는것을 기다려야 한다. 
...
샤드 키 값이 ‘H’ 보다 작은 경우 node1에 저장하고, ‘H’보다 크거나 같고 ‘T’보다 작은 경우 node2에 저장하고, ‘T’보다 크거나 같고 ‘Z’보다 작은 경우 node3에 저장한다.
...
복제 분산은 샤드 키를 적용한 데이터 분산이 아닌 샤드 객체가 복제되어 저장된 상태를 의미한다. 복제 분산을 적용하는 샤드 객체는 샤드 키가 필요없고, 모든 샤드 노드들에 저장된다.
...
```
iSQL> EXEC DBMS_SHARD.SET_SHARD_CLONE(‘user1’,‘t1’);
...
### 샤드 메타 테이블
분산 데이터베이스에 생성된 분산 객체에 대한 정보를 저장하고 있는 SYS_SHARD 사용자의 테이블이다.
...
##### SHARDED_DB_NAME
지역 샤드 노드가 참여할 논리적인 샤드된 데이터 베이스 이름을 나타내며, 지역 데이터 베이스의 DB 이름으로 자동으로 설정된다. 
...
- partition_key_column_name: 샤드 키 컬럼 이름
...
- user_name: 객체 소유자의 이름
- object_name: 객체 이름
- hash_max: 해시 분산의 상한값 [1-1000] (non-inclusive)
- node_name: 기본 샤드노드
...
- user_name: 객체 소유자의 이름
- object_name: 객체 이름
- value_max: 범위 분산 샤드 키의 상한값(non-inclusive)
- node_name: 기본 샤드 노드
...
- user_name: 객체 소유자의 이름
- object_name: 객체 이름
- old_node_name: 현재 노드 이름
- new_node_name: 변경할 노드 이름
- value: 샤드 키의 최대값
