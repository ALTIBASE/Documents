...
- 샤드 테이블들은 모두 생성되어 있되, 비어 있어야 한다.
- 백업테이블들인 \_BAK_ 테이블들은 모두 DROP해 놓았다면, 필요시 자동으로 만들어 진다.
- 백업테이블들인 \_BAK_ 테이블들 미리 생성해 놓아도 된다. 단, 모두 비어 있어야 한다.
...
새로운 샤드 노드를 추가하기 전에, 기존 샤드 노드에서 샤드테이블들간의 외래키가 있었다면, 기존 샤드 노드와 동일한 외래키를 미리 생성해 놓아야 한다.
- 이 경우에는, 백업테이블들인 \_BAK_ 테이블들에도 모두 생성되어 있어야 하며, 관련된 외래키들도 미리 생성해 놓아야 한다.
