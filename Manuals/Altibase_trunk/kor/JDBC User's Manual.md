...
| 필수 여부 | No                                                           |
| 설정 범위 | 세션                                                         |
| 설명      | TCP 소켓 옵션인 SO_LINGER의 활성화 여부를 설정한다.<br>- true: SO_LINGER 값을 0으로 설정한다. 소켓을 닫는 즉시 연결이 종료되고 남아있는 데이터는 전송되지 않는다.<br>- false: SO_LINGER을 비활성화한다. 소켓은 바로 닫히지만 시스템은 FIN_WAIT_2 상태에서 일정 시간 동안 아직 전송되지 않은 데이터를 보내려고 시도한다. |
