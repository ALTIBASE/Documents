...
이중화 연결을 할 때 지역 서버의 이중화 전용 IP 주소이다.<br/>
이 값이 설정되면 다중 네트워크로 구성되어 다른 사용 가능한 주소가 있더라도, 설정된 주소만 이중화에 사용한다.<br/>
지역 서버의 모든 네트워크 주소를 이중화에 사용하려면 이 프로퍼티를 ANY로 설정한다.<br/>
TCP를 제외한 다른 통신은 이 값을 무시한다.<br/>
