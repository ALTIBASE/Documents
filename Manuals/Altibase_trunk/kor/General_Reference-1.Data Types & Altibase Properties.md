...
값이 2인 경우는 디스크 공간 부족을 포함한 모든 예외 상황에서 트레이스 로그(altibase_sm.log)에 에러 메시지를 출력하고 온라인 로그파일을 백업하지 않는다. 값을 2로 설정 시 온라인 로그파일이 차례로 백업 되지 않고 일부가 빠질 수 있다. 또한 체크포인트 발생 시 백업 되지 않은 로그파일도 불필요한 로그 파일로 판단하고 삭제되기 때문에 운영 시 주의가 필요하다.
