...
정리되지 않은 분산 트렌젝션이 있을 때 해당 프로퍼티의 값을 감소시키고 서버를 재시작한 경우 기존 언두 테이블스페이스의 공간에 접근할 수 없으므로 서버가 정상적으로 뜨지 않는다.
이 경우 기존 값을 복원 한 후 재시작하여 분산 트렌젝션을 정리 한 후 서버를 내리고 프로퍼티 값을 변경해야 한다.
