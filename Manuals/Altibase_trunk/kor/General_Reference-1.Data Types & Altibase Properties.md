...
이중화 연결에 사용할 지역 서버의 TCP/IP 주소이다.<br/>
기본값 ANY는 지역 서버의 모든 IP 주소를 이중화 통신에 사용할 수 있다. ANY가 아닌 특정 IP를 설정하면 다중 IP 네트워크 환경이더라도 설정한 IP 주소만 이중화 통신에 사용한다.<br/>
이 설정은 이중화 통신 방법 TCP, Inifiband 중 TCP 만 적용된다.<br/>
설정 값이 유효하지 않으면 Altibase 서버 구동에 실패한다. 
