...
값이 0인 경우 아카이브로그 쓰레드는 오류 메시지를 출력한 후 아카이브 로그파일을 백업하는 작업을  처리하지 않고 다음 파일로 넘어간다.  이후 충분한 디스크 공간이 확보되면 새로 생성된 로그파일들은 백업을 계속 한다.  이 경우 체크포인트가 발생하면 아카이브 로그파일이 백업되지 않았더라도 불필요한 로그 파일들은 삭제되기 때문에 운영시 주의가 필요하다.
...
값이 2인 경우는 디스크 공간 부족 에러를 포함한 모든 에러 상황에서 오류 메시지를 출력한 후 아카이브 로그파일을 백업하는 작업을 처리하지 않고 다음 파일로 넘어간다.  이후 에러 상황이 해결 되면 새로 생성된 로그파일들은 백업을 계속 한다. 이 경우 체크포인트가 발생하면 아카이브 로그파일이 백업되지 않았더라도 불필요한 로그 파일들은 삭제되기 때문에 운영시 주의가 필요하다.
