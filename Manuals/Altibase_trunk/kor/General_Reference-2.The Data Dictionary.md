...
메모리 테이블 스페이스의 안정적인 체크포인트 이미지 파일 정보를 보여준다.
...
체크포인트 스케일 설정이 체크포인트 스케일 페어라면 이 값은 테이블스페이스의 안정적인 체크포인트 이미지 파일 중 파일 번호 0번을 보여준다. 

체크포인트 스케일 설정이 체크포인트 스케일 싱글이라면 이 값은 테이블스페이스의 모든 체크포인트 이미지 파일의 파일 번호를 보여준다.
...
체크포인트 스케일 설정이 체크포인트 스케일 페어라면 이 칼럼의 값은 0 혹은 1을 가진다.

체크포인트 스케일 설정이 체크포인트 스케일 싱글이라면 이 칼럼의 값은 -1로 나오며, 이 때 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 V$MEM_STABLE에서 확인할 수 있다.
