...
| -lightmode                                      | Direct-Path INSERT 방식이 아닌 일반적인 데이터 삽입에서 빠른 삽입을 위해 로그 파일을 기록하지 않는 옵션이다. 로그 파일을 기록하지 않기 때문에 데이터 삽입에 문제가 발생하면 대상 테이블의 정상적인 복구가 불가능할 수도 있다. 따라서 이러한 상황이 발생하면 대상 테이블을 재생성하는 것을 권장한다.<br />이 옵션은 iLoader의 병렬 수행을 지원한다. 따라서, 하나의 테이블에 대해 -parallel 옵션 사용 가능하며, 여러 개의 iLoader 인스턴스를 이용한 데이터 삽입도 가능하다. <br />하지만, 이 옵션으로 데이터 삽입을 수행하는 동안 다른 트랜잭션에서 대상 테이블에 SELECT를 제외한 DML을 수행할 수 없다.  따라서, 이 옵션은 이중화 대상 테이블에 사용이 불가능하며, 이 옵션으로 데이터 삽입이 수행중인 테이블에 대해 이중화 연결을 생성 할 수 없다.<br />이 옵션은 -direct 옵션과 함께 사용할 수 없다. |
