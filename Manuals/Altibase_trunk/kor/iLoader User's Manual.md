...
| -stmt_prefix [prefix_value]               | in/out 모드 수행시 iLoader가 생성하는 SQL 구문 앞에 사용자 지정값을 설정할 때 사용하는 옵션이다. 옵션 값을 입력하지 않으면 "NODE [META]"가 기본값으로 설정된다.<br />"NODE [DATA('NODE1')]" 형식은 in 모드에서는 사용할 수 없고, out 모드에서만 사용할 수 있다.<br />옵션 값을 입력하지 않거나 "NODE [META]" 형식을 사용시에, 접속한 노드에서만 in/out 이 되는 것이다. 그러므로, in 모드에서는 접속한 노드에 소속되지 않은 데이타는 입력되지 않고 에러가 발생한다. 그리고, out 모드에서는 접속한 노드의 데이타만 가져온다.<br /><br />예) iloader in -s 127.0.0.1 -u sys -p manager -f T1.fmt -d T1.dat -array 100 -atomic -stmt_prefix<br />생성되는 구문: NODE [META] INSERT INTO T1 VALUES (?, ?)<br />iloader out -s 127.0.0.1 -u sys -p manager -f T1.fmt -d T1.dat -stmt_prefix "NODE [DATA('NODE1')]"<br />생성되는 구문: NODE [DATA('NODE1')] SELECT I1, I2 FROM T1<br /><br />이 옵션은 데이터 업로드/다운로드에만 유효하다. |
