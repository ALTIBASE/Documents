...
*IF EXISTS*를 지정하면 *procedure_name*에 명시된 저장 프로시저가 존재하는 경우 제거한다.
...
*IF EXISTS*를 지정하면 *function_name*에 명시된 저장 함수가 존재하는 경우 제거한다.
...
*IF EXISTS*를 지정하면 *typeset_name*에 명시된 타입 세트가 존재하는 경우 제거한다.
...
*IF EXISTS*를 지정하면 *package_name*에 명시된 패키지 전체 또는 패키지 바디가 존재하는 경우 제거한다.
