...
Altibase의 더블 라이트 동작은 두 가지로 구분된다.

##### 디스크 데이터베이스에서의 더블 라이트

Altibase 시스템의 페이지 크기와 파일 시스템의 물리적 페이지 크기가 다를 경우, 디스크 I/O 수행 중에 Altibase 서버가 비정상적으로 종료하면 페이지가 온전하지 못한 상태로 남아있을 수 있다.

이런 현상을 방지하기 위하여 Altibase는 페이지를 플러시할 때, 같은 이미지를 디스크용 더블 라이트 파일에 저장한 후, 페이지 원래 위치에 다시 저장한다. 그리고 Altibase를 재구동할 때 더블 라이트 파일의 내용과 실제 페이지의 내용을 비교하여 손상된 페이지를 복구한다.

더블 라이트 기능은 디스크의 결함을 보완해 주지만, 시스템의 성능을 떨어뜨릴 수 있다. 이 기능은 사용자가 성능을 위해 사용하지 않을 수 있다.

##### 메모리 데이터베이스에서의 더블 라이트

체크포인트 이미지 파일을 관리하는 방식을 싱글로 설정했을 경우, 체크포인트 이미지 파일을 갱신하는 방법 중 하나를 의미한다. 메모리용 더블 라이트 파일은 더블 라이트 동작 중에 사용하는 버퍼 파일이다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트)를 참조한다.
...
- SYS_DOUBLE_WRITE: 메모리 테이블스페이스의 더티 페이지를 체크포인트 이미지에 기록할 때 메모리 페이지를 임시로 저장하는 더블 라이트 버퍼 파일
...
메모리 테이블들은 체크포인트를 수행하면 물리적으로 체크포인트 이미지 파일에 저장된다. 체크포인트 이미지 파일의 용도는 디스크 테이블스페이스의 데이터 파일의 그것과는 다르다. 디스크 테이블스페이스의 데이터 파일은 객체들을 저장하기 위한 것인 반면, 메모리 테이블스페이스의 체크포인트 이미지 파일은 객체들을 백업하기 위한 것이다. 체크포인트 이미지 파일은 데이터베이스 운영에 직접적으로 필요하지 않다. 하지만 백업 및 복구 시간을 단축하기 위해서는 반드시 필요하다.

체크포인트 시 메모리 공간의 페이지들은 운영 체제에 의해 지원되는 파일에 저장된다. Altibase의 체크포인트 방식은 "핑퐁 체크포인트"이다. 핑퐁 체크포인트란 체크포인트 수행 시 [핑퐁 번호](#pingpongnumber) 0번 또는 1번 체크포인트 이미지 파일 두 개를 번갈아 가며 사용하는 것을 의미한다. 또한 각 체크포인트 이미지 파일은 디스크 입출력 비용의 분산을 목적으로 다수의 작은 파일로 분리될 수 있다.
...
휘발성 테이블스페이스의 구조는 모든 데이터가 메모리 공간에 저장되는 메모리 테이블스페이스와 동일하다. 그러나 디스크상의 체크포인트 이미지를 가지지 않는다는 점에서 메모리 테이블스페이스와 차이가 있다. 휘발성 테이블스페이스의 데이터는 메모리에만 상주한다.
...
휘발성 테이블스페이스는 메모리에 데이터베이스 객체를 상주시킨다는 점에서 메모리 테이블스페이스와 동일하다. 그러나 휘발성 테이블스페이스는 체크포인트 이미지를 갖지 않는다.
...
체크포인트 이미지 파일 경로는 메모리 테이블스페이스에만 적용된다. Altibase는 메모리 테이블스페이스의 고성능 트랜잭션 처리를 위하여 핑퐁 체크포인트를 사용한다. 핑퐁 체크포인트를 위해서 두 개의 체크포인트 이미지 파일이 디스크에 생성된다. 체크포인트 이미지는 다수의 파일에 분할되어 저장될 수 있다. 체크포인트 이미지가 분할되는 크기는 SPLIT EACH절에 명시할 수 있다. 분할된 파일은 디스크 입출력 비용을 분산하기 위하여 서로 다른 경로에 저장될 수 있으며, 사용자가 임의로 분할의 크기 및 체크포인트 이미지 파일들이 저장되는 경로들을 지정할 수 있다. 사용자는 체크포인트 이미지 경로를 추가하거나 변경할 수 있지만, 한번 지정된 분할의 크기를 변경할 수는 없다.
...
이 파일들은 모두 메모리 테이블스페이스의 체크포인트 이미지 파일이다. 파일 이름의 형식은 ‘테이블스페이스이름-{Ping Pong번호}-{파일번호}’이다. ‘Ping Pong 번호’는 0또는 1로, 이는 두 개의 체크포인트 이미지 파일 중 핑퐁 체크포인트<sup>6</sup>시에 사용된 하나를 가리킨다. 또한, 각 체크포인트 이미지들을 여러 개의 파일로 나누어 기록하는데, 파일 이름의 맨 마지막 ‘파일번호’가 바로 나누어진 체크포인트 이미지 파일의 번호이며, 0부터 시작하여 1씩 증가한다. 체크포인트 이미지 파일 하나의 크기는 CREATE TABLESPACE 구문의 SPLIT EACH절에 지정된다. 위의 CREATE MEMORY TABLESPACE 구문에서는 SPLIT EACH절을 사용하지 않기 때문에, 기본값으로 DEFAULT_MEM_DB_FILE_SIZE 프로퍼티에 지정된 1GB 단위로 체크포인트 이미지 파일들이 분할될 것이다. 위 세 개의 테이블스페이스에 사용된 공간이 아직 1GB에 도달하지 않았기 때문에, 파일번호는 0까지만 존재하는 것을 볼 수 있다.

[<sup>6</sup>] Altibase는 메모리 상에 존재하는 테이블스페이스의 데이터의 영속성(Durability) 보장을 위해 디스크의 파일에 데이터를 저장한다. 이와 같이 테이블스페이스의 데이터가 저장되는 파일을 체크포인트 이미지 파일이라고 한다. Altibase가 사용하는 핑퐁 체크포인트 방식은 두 벌의 체크포인트 이미지 파일을 두고, 하나씩 번갈아 가면서 테이블스페이스의 데이터가 저장된다. ????? 페어에만 적용되는 설명이므로 삭제?(싱글의 경우 더블라이트에는 해당 안됨)
...
마지막으로 기존의 Altibase 홈 디렉터리의 dbs2안의 USER_MEM_TBS 테이블스페이스를 위한 체크포인트 이미지 파일들을 USER_MEM_TBS의 다른 체크포인트 경로중 하나로 옮겨주어야 한다.
...
다음 예제는 메모리 테이블스페이스인 USER_MEM_TBS를 생성한 후, 체크포인트 이미지 파일을 삭제한 상태에서 해당 테이블스페이스만 제외한 나머지 테이블스페이스들로 Altibase를 기동하는 과정을 보여준다.
...
핑퐁 체크포인트는 체크포인트 수행 시 체크포인트 이미지 파일에 [핑퐁 번호](#pingpongnumber)로서 0이나 1을 부여하고, 더티 페이지를 핑퐁 번호 0번과 1번 체크포인트 이미지 파일에 번갈아 쓰는 것이다. 이 때 더티 페이지의 내용이 모두 반영된, 마지막으로 기록된 체크포인트 이미지 파일을 안정적인 체크포인트 이미지 파일이라고 한다. 데이터베이스를 백업이나 복구할 때, 이 안정적인 체크포인트 이미지 파일을 사용한다.
...
하지만 Altibase는 체크포인트 대상 페이지에 래치를 획득하지 않고 체크포인트를 수행하여, 성능 저하를 해결하고 있다. 또한 체크포인트 도중 WAL 프로토콜을 지키지 않아서 발생할 수 있는 체크포인트 이미지 파일의 일관성 위배는 두 개의 체크포인트 이미지 파일을 유지하여 해결한다. 예를 들어 마지막으로 수행한 체크포인트 이미지 파일의 일관성이 깨진 상태로 Altibase 서버에 장애가 발생한 경우, 두 개의 체크포인트 이미지 파일 중 이전 이미지 파일을 사용해서 복구가 가능하다. ????? 이건 페어 한정 설명?

> [!NOTE]
>
> <a name="pingpongnumber"></a>핑퐁 번호란 데이터베이스를 백업 또는 복구할 때, 안정적인 체크포인트 이미지 파일을 구별하기 위해 부여하는 번호이다.
>
> 핑퐁 번호는 체크포인트 이미지 파일의 이름에서 테이블스페이스 이름 뒤에 위치한 숫자로 확인할 수 있다. 
>
...
> ````
>
> 예를 들어, `MY_MEM_TBS-0-0` 라는 체크포인트 이미지 파일의 이름에서 사용자는 이것이 *MY_MEM_TBS*라는 메모리 테이블스페이스의 핑퐁 번호 0번의, 첫 번째로 생성된 체크포인트 이미지 파일인 것을 알 수 있다 .

##### 체크포인트 스케일(Checkpoint Scale)

체크포인트 스케일은 Altibase가 체크포인트 이미지 파일을 생성, 갱신 및 유지하는 관리 방식이다. 사용자는 데이터베이스를 생성할 때 체크포인트 스케일을 설정할 수 있고, 데이터베이스 생성 이후에는 Altibase 서버 구동 단계 중 CONTROL 단계에서 체크포인트 스케일을 변경할 수 있다.

Altibase는 다음의 두 가지 체크포인트 스케일을 지원한다.

###### 체크포인트 스케일 페어(Checkpoint Scale Pair)

메모리 테이블스페이스 생성 시 핑퐁 번호 0번과 1번을 부여받은 체크포인트 이미지 파일 두 개도 같이 생성하고, 체크포인트를 수행 할 때마다 더티 페이지를 두 체크포인트 이미지 파일에 번갈아 기록한다. 

데이터베이스 생성 시 체크포인트 스케일을 따로 설정하지 않았다면 Altibase는 페어 방식으로 체크포인트 이미지 파일을 관리한다.

예를 들어 체크포인트 스케일이 페어로 설정된 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지 파일은 다음과 같이 관리된다.

```bash
#dbs 디렉터리에서 메모리 테이블스페이스 MY_MEM_TBS의 체크포인트 이미지 파일을 조회한다
% ls -al | grep 'MY_MEM_TBS'
MY_MEM_TBS-0-0
MY_MEM_TBS-1-0
MY_MEM_TBS-0-1
MY_MEM_TBS-1-1
...
위의 결과에서 사용자는 Altibase가 다음과 같이 *MY_MEM_TBS*의 체크포인트 이미지 파일을 한 쌍을 유지하여 관리한다는 것을 알 수 있다. 즉, 체크포인트 이미지 파일 MY_MEM_TBS-0-0에 기록된 페이지는 MY_MEM_TBS-1-0에도 기록되어 있는 것이다.

| 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 |
| ------------------------------- | ------------------------------- |
| MY_MEM_TBS-0-0 | MY_MEM_TBS-1-0 |
| MY_MEM_TBS-0-1       | MY_MEM_TBS-1-1     |
| MY_MEM_TBS-0-2    | MY_MEM_TBS-1-2      |

페어 방식으로 체크포인트 이미지 파일을  관리하면 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 테이블스페이스마다 다르다. 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알고 싶다면 [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla)를 사용하여 로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목을 살펴봐야 한다.

###### 체크포인트 스케일 싱글(Checkpoint Scale Single)

체크포인트 스케일을 싱글로 설정하면 Altibase는 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만, 결과적으로 각 메모리 테이블스페이스를 구성하는 페이지마다 안정적인 체크포인트 이미지 파일 하나만 남게 된다. 이는 디스크 사용량의 효율성을 높일 수 있다.

예를 들어 체크포인트 스케일이 싱글로 설정된 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지 파일은 다음과 같이 관리된다.

```bash
#dbs 디렉터리에서 MY_MEM_TBS 메모리 테이블스페이스의 체크포인트 이미지 파일을 조회한다
% ls -a1 | grep 'MY_MEM_TBS'
MY_MEM_TBS-0-0
MY_MEM_TBS-1-1
MY_MEM_TBS-0-2
```

위의 결과에서 사용자는 Altibase가 다음과 같이 *MY_MEM_TBS*를 구성한 페이지마다 안정적인 체크포인트 이미지 파일을 하나만 유지하고 있는 것을 알 수 있다. 즉, MY_MEM_TBS-0-0에 기록된 페이지 내용은 이 파일에만 존재하지만, 안정적인 체크포인트 이미지 파일이기 때문에 가장 최근의 변경 내용을 반영하고 있다는 뜻이다.
...
싱글 방식으로 체크포인트 이미지를 관리하면 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 체크포인트 이미지 파일마다 다르다. 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알고 싶다면 [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla)를 사용하여 로그 앵커의 "[ MEMORY CHECKPOINT IMAGE ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목을 살펴봐야 한다. ????? 여기 설명 어차피 남은 건 다 안정적인 이미지 파일인데 굳이 핑퐁번호를 조회하여 알아야 할 이유가 있을까요?

**체크포인트 스케일 싱글에서 체크포인트 이미지 파일 기록 방식**

체크포인트 스케일을 싱글로 설정했을 경우, 체크포인트 이미지 파일에서 갱신되어야 할 페이지의 비율에 따라 기록하는 방법이 달라진다.  풀 라이트(Full Write) 방식을, 그렇지 않을 경우 더블 라이트(Double Write) 방식을 선택한다. 

- 풀 라이트: 더티 페이지가 체크포인트 이미지 파일의 절반(50%)을 초과하는 경우의 기록 방식이다.
...
- 더블 라이트: 더티 페이지가 체크포인트 이미지 파일의 절반(50%) 이하인 경우의 기록 방식이다.

  1. 메모리용 더블 라이트 버퍼에 더티 페이지의 스냅샷을 저장한다.

  2. 1에서 저장한 스냅샷을 메모리용 더블 라이트 파일에 기록한다.

  3. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다.

  4. 메모리용 더블 라이트 버퍼에 저장한 스냅샷을 안정적인 체크포인트 파일에 직접 기록한다.

     > [!CAUTION]
     >
     > 더블 라이트 방식에서 사용하는 더블 라이트 버퍼 혹은 파일을 디스크 I/O 수행 시 사용하는 디스크용 더블 라이트 파일과 혼동하지 않도록 주의한다. 

     > [!NOTE]
     >
     > **더블 라이트 리커버리(Double Write Recovery)**
     >
     > 더블 라이트 방식은 더티 페이지를 안정적인 체크포인트 이미지 파일에 직접 기록한다. 이 과정에서 장애가 발생하면 부분 쓰기(partial write)로 인한 페이지 손상이 발생 할 수 있다. **더블 라이트 리커버리는 더블 라이트 파일에 보관된 더티 페이지의 스냅샷을 이용하여 손상되었을 가능성이 있는 페이지를 복구하는 동작이다.** 이 동작은 Altibase의 META 구동 단계에서 자동으로 수행된다.
     >
     > 손상된 체크포인트 이미지 파일을 완전히 복구하려면 복구된 더티 페이지를 체크포인트 이미지 파일에 기록해야 한다. 따라서 사용자는 장애가 발생한 서버를 다시 구동하여 더블 라이트 리커버리가 동작하게 한 후, 체크포인트를 수행해야 한다. 체크포인트를 수행하는 대표적인 구문은 다음과 같다.
     >
     > ```sql
