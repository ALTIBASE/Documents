...
하지만 Altibase는 체크포인트 대상 페이지에 래치를 획득하지 않고 체크포인트를 수행하여 성능 저하를 해결하고 있다. 이 때 체크포인트 도중 WAL 프로토콜을 지키지 않아서 발생할 수 있는 체크포인트 이미지 파일의 일관성 위배는 두 개의 체크포인트 이미지 파일에 기록하여 해결한다. 
...
> ```
>
> 핑퐁 번호는 0또는 1로, 체크포인트를 수행할 때 두 개의 체크포인트 이미지 파일을 번갈아 기록하기 위해 사용하는 번호를 의미한다. 핑퐁 번호로 메모리 데이터베이스를 백업하거나 복구할 때 안정적인 체크포인트 이미지 파일을 식별할 수 있다.
>
...
>
> 예를 들어, `MY_MEM_TBS-0-0` 라는 체크포인트 이미지 파일의 이름에서 사용자는 이 파일이 *MY_MEM_TBS*라는 메모리 테이블스페이스의 체크포인트 이미지 파일이고, 핑퐁 번호는 0번이며, 첫 번째로 생성된 체크포인트 이미지 파일인 것을 알 수 있다 .
...
체크포인트 스케일을 페어(PAIR)로 설정하면, 메모리 테이블스페이스를 생성할 때 핑퐁 번호 0번과 1번을 부여받은 두 개의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행 할 때마다 더티 페이지를 두 체크포인트 이미지 파일에 번갈아 기록한다. 
...
위의 결과에서 *MY_MEM_TBS*를 구성하는 체크포인트 이미지 파일은 안정적인 체크포인트 이미지 파일 하나만 유지하고 있는 것을 알 수 있다. 즉, *MY_MEM_TBS-0-0*는 안정적인 체크포인트 이미지 파일이며 가장 최근의 변경 내용을 반영하고 있다.
...
체크포인트 스케일을 싱글로 설정하면 안정적인 체크포인트 이미지 파일만 데이터베이스에 남는다. 따라서 사용자는 파일의 이름을 통해 안정적인 체크포인트 이미지의 핑퐁 번호를 알 수 있다. 다만 ??한 경우 안정적인 체크포인트 이미지의 핑퐁 번호를 따로 확인해야 할 수도 있다. 이럴 경우 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 아래와 같은 방법으로 확인할 수 있다.
