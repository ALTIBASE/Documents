...
>
> 파일 번호는 체크포인트 이미지 파일의 생성 순서를 나타내는 번호로, 0부터 시작하여 1씩 증가한다. 체크포인트 이미지의 크기가 지정한 단위보다 커져 새로운 체크포인트 이미지 파일이 생성될 때 순차적으로 부여된다.
>
...
Altibase는 각 메모리 테이블스페이스의 체크포인트 이미지를 SPLIT EACH 절에 지정한 크기만큼 나누어 체크포인트 이미지 파일에 기록한다. 즉, 체크포인트 이미지의 크기가 지정한 크기보다 커지면 체크포인트 이미지를 여러 파일로 분할한다. 분할된 체크포인트 이미지 파일은 생성된 순서대로 0부터 1씩 증가하는 파일 번호를 부여받는다. 체크포인트 이미지 파일 이름에 관한 좀 더 자세한 설명은 [여기](#checkpointimagefilenaming)를 참고하라.
...
만약 테이블스페이스의 데이터가 빈번하게 변경되어 체크포인트 시 디스크 I/O의 양이 증가했다면, 이는 다음과 같이 기존의 체크포인트 이미지 파일 경로와 물리적으로 다른 디스크의 경로를 추가해서 경감시킬 수 있다.
...
dbs 디렉터리에서 메모리 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지 파일을 조회하였다.
...
- 성능 뷰 조회
  ...
  [V$MEM_TABLESPACES](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_tablespaces) 혹은 [V$MEM_STABLE](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_stable) 성능 뷰의 CURRENT_DB 값을 확인한다.
...
- [V$MEM_STABLE](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_stable) 성능 뷰의 CURRENT_DB 값 확인

변경안 1 ----

위의 결과에서 확인할 수 있는 *MY_MEM_TBS*의 체크포인트 이미지 파일은 전부 안정적인 체크포인트 이미지 파일이며 가장 최근의 변경 내용을 반영하고 있다.

기본적으로 사용자는 파일의 이름을 통해 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알 수 있다. 그러나 파일이 유실되거나 손상되어 정확한 확인이 필요할 경우, 아래 방법을 사용하여 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 확인할 수 있다.
...
위의 결과에서 *MY_MEM_TBS*를 구성하는 체크포인트 이미지 파일은 안정적인 체크포인트 이미지 파일 하나만 유지하고 있는 것을 알 수 있다. 즉, 각각의 파일은 모두 안정적인 체크포인트 이미지 파일이며 가장 최근의 변경 내용을 반영하고 있다. 사용자는 파일의 이름을 통해 현재 데이터베이스의 안정적인 체크포인트 이미지의 핑퐁 번호를 알 수 있다.

특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 아래와 같은 방법으로 확인할 수 있다.
...
- **더블 라이트**
  ...
  더티 페이지가 체크포인트 이미지 파일의 50% 이하일 때 기록하는 방식이다. 
...
더블 라이트 방식을 사용할 경우, Altibase는 더티 페이지를 안정적인 체크포인트 이미지 파일에 직접 기록한다. 이 때 장애가 발생하면 부분 쓰기(Partial write)로 안정적인 체크포인트 이미지 파일이 손상될 수 있다. 이와 같은 상황이 발생하면, 기록 중이던 더티 페이지를 복구한 후 안정적인 체크포인트 이미지 파일에 다시 기록해야 한다. 

체크포인트 이미지 파일 복구 절차는 다음과 같다.

1. 더티 페이지 복구

   장애가 발생한 시점에 기록 중이던 더티 페이지를 더블 라이트 이미지 파일을 이용하여 복구한다. 이 작업은 장애가 발생한 Altibase 서버를 구동하면 META 구동 단계에서 자동으로 수행하며, 이를 더블 라이트 리커버리(Double Write Recovery)라고 한다.

2. 체크포인트 수행

   사용자가 명시적으로 체크포인트를 수행하여, 복구 된 더티 페이지를 체크포인트 이미지 파일에 기록한다. 체크포인트를 명시적으로 수행하는 대표적인 구문은 다음과 같다.
...
더블 라이트 리커버리는 서버 구동 시 META 구동 단계에서 자동으로 수행된다.

다음과 같은 오류 메시지가 나타나면 더블 라이트 리커버리를 이용한 체크포인트 이미지 파일 복구가 필요하다:

```
[ERR-#### : The process was interrupted while performing a Scale SINGLE Checkpoint.]
```
...
1. 서버 재구동:
   ...
   장애가 발생한 서버를 다시 구동한다. 이때 더블 라이트 리커버리가 자동으로 수행된다.

2. 체크포인트 수행:

   구동이 완료된 후, 서버에서 체크포인트를 수행한다. 체크포인트는 자동 또는 수동으로 수행할 수 있으며, 최소 한 번은 반드시 수행되어야 한다.

   체크포인트를 수동으로 수행하는 대표적인 구문은 다음과 같다.
