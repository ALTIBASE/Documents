...
메인 메모리 데이터베이스에서의 퍼지 체크포인트(fuzzy checkpoint)는 모든 변경된 데이터 페이지가 백업 데이터베이스로 내려가 현재 수행중인 트랜잭션의 수행에 영향을 미칠 수 있기 때문에 퍼지 체크포인트와 더불어 핑퐁 체크포인트(ping pong checkpoint) 방식을 함께 수행한다. 즉, 백업 데이터베이스를 두 개로 관리함으로써, 체크포인트 과정에서 부하를 줄일 수 있어 트랙잭션 동작이 최대한의 성능을 발휘할 수 있다. 핑퐁 체크포인트를 수행하면서 백업 데이터베이스를 한개만 유지 관리하는 방법 또한 지원하여 디스크 사용량의 효율성을 높일 수 있다.
...
Altibase 는 두 개의 체크포인트 이미지 파일을 유지하는 체크포인트 방식(Checkpoint Scale PAIR) 이외에도 핑퐁 체크포인트 방식을 사용하면서 하나의 체크포인트 이미지만을 유지하는 체크포인트 방식(Checkpoint Scale SINGLE)을 추가로 제공한다. 이때 데이터베이스의 내용을 백업 데이터 파일에 저장하는 과정에서 백업 데이터 파일 단위의 핑퐁 체크포인트 후 가장 최신의 백업 이미지 파일만을 남기는 방식(Full Write)과 더블 라이트 파일이라는 임시 파일을 이용하여 핑퐁 체크포인트와 동일한 효과를 보장하는 방식(Double Write)을 사용한다.

##### 핑퐁 체크포인트 이미지 유지 방식

###### Checkpoint Scale PAIR

두벌의 체크포인트 이미지 파일 (0번, 1번)을 유지하며, 핑퐁 체크포인트를 수행 할 때마다 수정된 페이지를 각 체크포인트 이미지 파일의 0번, 1번 파일에 번갈아 기록한다. 이때, 기록되는 체크포인트 이미지 파일을 안정적(stable) 이미지라고 하며, 안정적인 파일을 가르키는 핑퐁 번호는 로그 앵커에 저장된다.

안정적인 체크포인트 이미지의 핑퐁 번호는 테이블스페이스 단위로 관리되며, 로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목으로 저장된다.

예를 들어 MY_TBS 라는 테이블스페이스에 해당하는 체크포인트 이미지 파일은 다음과 같이 두벌의 이미지 파일로 디스크에 유지한다.
...
핑퐁 체크포인트를 수행하면서 가장 최신의 체크포인트 이미지 1개만 유지하기 위한 방법으로, 체크포인트 이미지 파일 단위로 안정적인(stable) 체크포인트 이미지에 대한 핑퐁 번호를 관리한다. 체크포인트 이미지 기록을 위한 방법에는 두가지 방식을 자동으로 선택하여 사용한다. 방식을 선택하는 기준은 체크포인트 이미지 파일에서 변경된 페이지의 비율이다. 변경된 페이지가 체크포인트 이미지 파일의 절반(50%) 이하인 경우 Double Write 방식을 사용하고, 초과하는 경우는 Full Write 방식을 선택한다.

안정적인 체크포인트 이미지의 핑퐁 번호는 체크포인트 이미지 파일 단위로 관리되며, 로그 앵커의 "[ MEMORY CHECKPOINT PATH ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목으로 저장된다.

예를 들어 MY_TBS 라는 테이블스페이스에 해당하는 체크포인트 이미지 파일은 다음과 같이 각각의 안정적인 체크포인트 이미지 파일 1개를 디스크에 유지한다.
...
안정적이지 않은(unstable) 체크포인트 이미지 파일을 생성하고 그에 해당하는 모든 페이지를 기록한다. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다. 로그 앵커에서 안정적인(stable) 체크포인트 이미지 파일에 대한 핑퐁 번호를 변경한다. 이제 생성된 체크포인트 이미지 파일은 안정적인(stable) 체크포인트 이미지 파일이 되었다. 기존의 체크포인트 이미지 파일을 삭제하면 최신의 안정적인(stable) 체크포인트 이미지 파일만 남게 된다.

더블 라이트(Double Write):

WAL 프로토콜을 구현하기 위하여 더블 라이트 버퍼(Double Write Buffer) 라는 메모리 자료구조에 수정된 페이지의 스냅샷을 보관한다. 더블 라이트 파일(Double Write File) 이라는 임시 파일에 수정된 페이지를 기록한다. 로그 레코드를 기록하고 더블 라이트 버퍼에 보관된 수정된 페이지의 스냅샷을 안정적인(stable) 체크포인트  파일에 기록한다. 

더블 라이트 방식은 Checkpoint Scale SINGLE 방식의 핑퐁 체크포인트를 수행할 때 소량의 변경된 페이지를 풀 라이트 방식보다 효율적으로 체크포인트 이미지에 기록하기 위해 사용한다.

더블 라이트 리커버리(Double Write Recovery):

핑퐁 체크포인트에서는 안정적인(stable) 체크포인트 이미지에 기록하지 않고 안정적이지 않은(unstable) 체크포인트 이미지 파일에 기록한다. 하지만 더블 라이트 방식에서는 더블 라이트 파일에 수정된 페이지를 기록하고 WAL 프로토콜을 구현하였기 때문에 안정적인 체크포인트 이미지에 직접 수정된 페이지를 기록할 수 있다. 

만약 안정적인 체크포인트 이미지에 기록하는 시점에 장애가 발생하여 해당 체크포인트 이미지 파일이 손상되더라도 더블 라이트 파일에 기록된 페이지 복사본을 이용하여 안정적인 체크포인트 이미지 파일을 복구 할 수 있다. 이 과정은 더블 라이트 리커버리(Double Write Recovery)라고 한다.

더블 라이트 리커버리는 Altibase 의 Meta 구동 단계에서 수행된다. 따라서 더블 라이트 동작 과정에서 장애가 발생한 경우 Altibase의 구동을 통해 복구할 수 있다. 더블 라이트 리커버리로 복구된 메모리 페이지는 체크포인트 이미지에 반영되어야 복구가 완료된다.

체크포인트 이미지에 복구된 페이지가 기록되기 위해서는 체크포인트가 수행되어야 한다. 이것은 운용자의 명시적인 체크포인트 수행 또는 주기적인 체크포인팅, 그리고 Altibase 서버의 정상 종료와 같은 동작에서 묵시적으로 동작하기도 한다. 따라서 더블 라이트 리커버리를 즉시 완료하기 위해서는 다음과 같은 동작 중 하나를 수행해야 한다.

* ALTER SYSTEM CHECKPOINT 수행
* SHUTDOWN NORMAL 수행
* SHUTDOWN IMMEDIATE 수행
...
| [CHECKPOINT-SCALE] PAIR                               | 핑퐁 체크포인트의 동작 방식 확인                             |
...
데이터베이스의 백업 데이터 파일은 MEM_DB_DIR 프로퍼티에 설정한 위치에 저장된다. 

2개의 백업 데이터 파일을 유지하는 핑퐁 체크포인트 (ping-pong checkpoint) 기법을 사용하는 Checkpoint Scale PAIR 방식인 경우, 동일한 이미지를 기록한 1묶음(pair)의 데이터 파일 2개 모두 존재해야만 Altibase가 정상적으로 운용된다. 어느 한 시점에서는 메모리 테이블스페이스는 이 묶음 중 1개의 데이터 파일만 사용한다.

백업 수행 시간을 줄이려면 메모리 테이블스페이스의 가장 최근 체크포인트가 수행된 2개의 데이터 파일 묶음 중 1개의 데이터 파일만 백업한다.

한 개의 백업 데이터 파일을 유지하는 핑퐁 체크포인트 기법을 사용하는 Checkpoint Scale SINGLE 방식인 경우, 가장 최근에 체크포인트가 수행된 1개의 데이터 파일만 유지한다. 백업 데이터 파일을 선별 과정 없이 백업에 사용한다.
...
안정한 데이터 파일의 핑퐁 번호를 확인하기 위해서는 \$ALTIBASE_HOME/bin/dumpla loganchor0 결과 중 loganchor 헤더와 SYS_TBS_MEM_DIC 테이블스페이스의 테이블스페이스 속성, 메모리 체크포인트 이미지 속성을 확인해야 한다.

먼저 Loganchor 헤더의 Checkpoint Scale 항목을 참고하여 Checkpoint 방식을 구분한다.

Checkpoint Scale PAIR 방식을 사용중인 경우 테이블스페이스 속성의 Stable Checkpoint Image Num. 항목을 참고한다.

Checkpoint Scale SINGLE 방식을 사용중인 경우 메모리 체크포인트 이미지 속성의 Stable Single Checkpoint Image Num. 항목을 참고한다.
