...
메모리 데이터베이스에서의 더블 라이트란 체크포인트 이미지를 관리하는 방식을 싱글로 설정했을 경우에 체크포인트 이미지를 갱신 또는 생성하는 방법들 중 하나를 의미한다. 메모리용 더블 라이트 파일은 이 더블 라이트 동작 중에 사용하는 버퍼 파일을 의미한다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트)를 참조한다.
...
체크포인트 스케일은 체크포인트 이미지 파일을 관리하는 방식이다. 사용자는 Altibase 서버 구동 단계 중 CONTROL 단계에서 체크포인트 스케일을 설정할 수 있다. Altibase는 다음의 두 가지 체크포인트 스케일을 지원한다.
...
- 더블 라이트: 더티 페이지가 체크포인트 이미지 파일의 절반(50%) 이하이인 경우의 기록 방식이다.

  	1. 메모리용 더블 라이트 버퍼에 더티 페이지의 스냅샷을 저장한다.
  	1. 1에서 저장한 스냅샷을 메모리용 더블 라이트 파일에 기록한다.
  	1. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다.
  	1. 메모리용 더블 라이트 버퍼에 저장한 스냅샷을 안정적인 체크포인트 파일에 직접 기록한다.
...
체크포인트 스케일이 싱글로 설정되어 있을 경우, Altibase는 가장 최근에 체크포인트가 수행된 1개의 체크포인트 이미지 파일만 유지한다. 따라서 선별 과정 없이 그 체크포인트 이미지 파일을 백업하여 매체 복구를 수행한다.
