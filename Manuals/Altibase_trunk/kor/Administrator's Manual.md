...
.dwf 파일과 SYS_DOUBLE_WRITE 파일은 더블 라이트 버퍼 파일로서 디스크 페이지 및 메모리 페이지가 임시로 저장된다.
...
메모리 테이블들은 체크포인트 수행를 수행하면 물리적으로 체크포인트 이미지 파일에 저장된다. 체크포인트 이미지 파일의 용도는 디스크 테이블스페이스의 데이터 파일의 그것과는 다르다. 디스크 테이블스페이스의 데이터 파일은 객체들을 저장하기 위한 것인 반면, 메모리 테이블스페이스의 체크포인트 이미지 파일은 객체들을 백업하기 위한 것이다. 체크포인트 이미지 파일은 데이터베이스 운영에 직접적으로 필요하지 않다. 하지만 백업 및 복구 시간을 단축하기 위해서는 반드시 필요하다.

체크포인트 시 메모리 공간의 페이지들은 운영 체제에 의해 지원되는 파일에 저장된다. Altibase의 체크포인트 방식은 "핑퐁 체크포인트"라고 불리며, 체크포인트 수행 시 두 개의 체크포인트 이미지 파일(핑퐁 번호 0과 1)을 번갈아 가며 사용하는 방식이다. 체크포인트를 수행할 때마다 0번 파일과 1번 파일을 교대로 생성 또는 갱신하여 두 그룹의 체크포인트 이미지 파일을 유지한다. 또한 각 체크포인트 이미지 파일은 디스크 입출력 비용의 분산을 목적으로 다수의 작은 파일로 분리될 수 있다.

> [!TIP]
>
> <a name="pingpongnumber"></a>핑퐁 번호란 핑퐁 체크포인트로 인해 생성 또는 갱신되는 체크포인트 이미지 파일을 관리하기 위하여 Altibase가 체크포인트 이미지 파일에 부여하는 번호로서, 0과 1을 사용하여 두 그룹으로 파일을 구분한다. 핑퐁 번호는 체크포인트 이미지 파일의 이름에서 테이블스페이스 뒤에 위치한 숫자를 통해 확인할 수 있다. 체크포인트 이미지 파일의 이름 규칙은 다음과 같다.
>
> ````
> 테이블스페이스 이름-{핑퐁 번호}-{파일 번호}
> ````
>
> 예를 들어, `MY_MEM_TBS-0-0` 라는 파일은 MY_MEM_TBS라는 이름의 메모리 테이블스페이스의 체크포인트 이미지 파일이며, 핑퐁 번호 0번 파일이라는 것을 나타낸다.
...
핑퐁 체크포인트는 체크포인트 이미지 파일을 [핑퐁 번호](#pingpongnumber) 0번 그룹과 1번 그룹으로 나누고, 체크포인트를 수행할 때마다 더티 페이지를 각 그룹의 이미지 파일에 번갈아 쓰는 것이다. 
...
사용자는 핑퐁 체크포인트를 수행할 때 체크포인트 스케일(Checkpoint Scale)을 설정할 수 있다. 체크포인트 스케일은 핑퐁 체크포인트를 수행할 때 체크포인트 이미지를 유지하는 방식이다. Altibase는 다음의 두 가지 체크포인트 스케일을 지원한다.

##### 체크포인트 스케일

###### 체크포인트 스케일 페어

핑퐁 체크포인트를 수행 할 때마다 수정된 페이지를 각 체크포인트 이미지 파일 그룹에 번갈아 기록하고, 파일들을 계속 유지한다. 따라서 체크포인트 스케일을 페어로 설정하면 같은 형상을 가진 체크포인트 이미지 파일이 핑퐁 번호 0번과 1번 그룹에 각각 하나씩, 총 2개로 쌍을 이루게 된다. 데이터베이스 생성 시 체크포인트 스케일을 설정하지 않았다면 핑퐁 체크포인트는 체크포인트 스케일 페어 방식으로 동작한다.

가장 최근에 갱신 또는 생성된 체크포인트 이미지 파일을 안정적인 이미지라고 하며, Altibase는 이 안정적인 체크포인트 이미지의 핑퐁 번호를 테이블스페이스 단위로 관리한다. 안정적인 체크포인트 이미지의 핑퐁 번호는 로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목에 저장된다. 

예를 들어 MY_TBS라는 테이블스페이스에 해당하는 체크포인트 이미지 파일은 다음과 같이 **쌍을 이루어** 디스크에 유지된다.

| 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 |
| ------------------------------- | ------------------------------- |
| MY_TBS-0-0                      | MY_TBS-1-0                      |
| MY_TBS-0-1                      | MY_TBS-1-1                      |
| MY_TBS-0-2                      | MY_TBS-1-2                      |

###### 체크포인스 스케일 싱글

핑퐁 체크포인트를 수행할 때마다 수정된 페이지를 체크포인트 이미지 파일 그룹에 번갈아 기록하되, 가장 최신의 체크포인트 이미지 1개만 유지한다. 이 방식은 디스크 사용량의 효율성을 높일 수 있다.

Altibase는 안정적인 체크포인트 이미지의 핑퐁 번호를 체크포인트 이미지 파일 단위로 관리하며, 로그 앵커의 "[ MEMORY CHECKPOINT IMAGE ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목에 저장한다.

예를 들어 MY_TBS 라는 테이블스페이스에 해당하는 체크포인트 이미지 파일은 다음과 같이 **안정적인 체크포인트 이미지 파일 1개가** 유지된다.

| 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 |
| ------------------------------- | ------------------------------- |
| MY_TBS-0-0                      | -                               |
| (MY_TBS-0-0 파일은 삭제된다)    | MY_TBS-1-1                      |
| MY_TBS-0-2                      | (MY_TBS-1-1 파일은 삭제된다)    |

체크포인트 스케일을 싱글로 설정했을 경우, 체크포인트 이미지 파일에서 갱신되어야 할 페이지의 비율에 따라 기록하는 방법이 달라진다. 더티 페이지가 체크포인트 이미지 파일의 절반(50%)을 초과할 경우 풀 라이트(Full Write) 방식을, 그렇지 않을 경우 더블 라이트(Double Write) 방식을 선택한다. 

- 풀 라이트

  우선 안정적이지 않은 체크포인트 이미지 파일을 새로 생성한다. 이 파일에 가장 최근 갱신된 내용을 포함한 기존의 체크포인트 이미지의 내용을 기록한다. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한 뒤, 로그 앵커에서 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 방금 생성한 체크포인트 파일의 핑퐁 번호로 변경한다. 이제 생성된 체크포인트 이미지 파일이 안정적인 체크포인트 이미지 파일이 되었다. 마지막으로 기존의 체크포인트 이미지 파일을 삭제하여 가장 최신의 안정적인 체크포인트 이미지 파일만 남긴다.

- 더블 라이트

  더블 라이트 버퍼(Double Write Buffer)라는 메모리 자료구조에 더티 페이지의 스냅샷을 보관하고, 더블 라이트 파일(Double Write File)이라는 임시 파일에도 이 스냅샷을 기록한다. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한 뒤, 마지막으로 더블 라이트 버퍼에 보관된 더티 페이지의 스냅샷을 안정적인 체크포인트 파일에 직접 기록한다.
  
  > [!CAUTION]
  >
  > 핑퐁 체크포인트 수행 시 더티 페이지의 스냅샷이 기록되는 더블 라이트 버퍼와 더블 라이트 파일은 디스크 I/O 수행 시 기록하는 더블 라이트 파일과 엄격하게 구분된다. 디스크 I/O 수행 시 사용되는 더블 라이트 파일은 디스크 테이블스페이스의 데이터 객체에 접근 또는 갱신하기 위해 임시로 사용하는 버퍼 풀의 내용을 저장하는 파일로서, Altibase 프로퍼티인 DOUBLE_WRITE_DIRECTORY에 지정한 디렉토리에 저장된다. 반면 핑퐁 체크포인트 수행 시 사용하는 더블 라이트 버퍼와 더블 라이트 파일은 ~
  
  > [!NOTE]
  >
  > **더블 라이트 리커버리(Double Write Recovery)**
  >
  > 더블 라이트 방식의 체크포인트를 수행하는 과정에서, 더티 페이지를 안정적인 체크포인트 이미지 파일에 기록하던 중에 장애가 발생하면 파일이 손상될 수 있다. 손상된 체크포인트 이미지 파일을 복구하려면 더티 페이지를 복구하는 과정이 선행되어야 한다. **더블 라이트 리커버리는 더블 라이트 파일에 보관된 스냅샷을 이용하여 더티 페이지를 복구하는 동작이다.** 이 동작은 Altibase의 Meta 구동 단계에서 자동으로 수행된다.
  >
...
| [CHECKPOINT-SCALE] PAIR                               | 핑퐁 체크포인트의 스케일 설정 확인                           |
...
**메모리 테이블스페이스의 데이터 파일 복구 시 안정적인 메모리 데이터 파일을 이용해야 한다.**

메모리 데이터베이스의 백업 데이터 파일은 MEM_DB_DIR 프로퍼티에 설정한 위치에 저장된다. 

체크포인트 스케일이 페어로 설정되어 있을 경우, 동일한 이미지를 기록한 한 쌍의 체크포인트 이미지 파일이 모두 존재해야만 Altibase가 정상적으로 운용된다. 어느 한 시점에서는 메모리 테이블스페이스는 이 중 1개의 데이터 파일만 사용한다.

백업 수행 시간을 줄이려면 메모리 테이블스페이스의 가장 최근 체크포인트 이미지 파일들 중 핑퐁 번호 하나를 정하여 그 번호의 파일만 백업한다.

백업 받은 메모리 테이블스페이스의 체크포인트 이미지 파일이 다음과 같다면:
...
메모리 테이블스페이스를 위한 체크포인트 이미지 파일은 다음과 같이 복사해야 한다.
...
위 결과는 백업된 데이터 파일을 사용하여 데이터베이스를 복원하기 위해서는 logfile4 아카이브 로그 파일 이후의 체크포인트 이미지 파일들이 필요함을 나타낸다.
