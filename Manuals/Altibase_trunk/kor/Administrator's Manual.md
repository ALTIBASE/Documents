...
| 전체 데이터베이스의 온라인 백업   | SQL 문 이용 ALTER DATABASE BACKUP DATABASE TO ‘BACKUP_DIR’;  | 시스템 전체 테이블스페이스의 데이터 파일과 로그 앵커파일 | 1\> 유닉스 명령어 cp 이용 2\> ALTER DATABASE RECOVER DATABASE; | O                |
...
체크포인트 스케일이 페어일 때, 체크포인트 이미지 파일은 핑퐁 번호 0번과 1번, 한 벌씩 존재한다. 매체 복구를 수행할 때 안정적인 체크포인트 이미지 파일을 이용해야 하기 때문에, 백업 시 사용자는 안정적인 체크포인트 이미지 파일을 확인 후 그 파일을 백업해야 한다.
...
체크포인트 스케일이 싱글이라면, 안정적인 체크포인트 이미지 파일 하나만 유지되기 때문에 백업 시 추가적인 파일 확인 과정이 필요하지 않다. 따라서 별도의 선별 작업 없이 백업한 파일들로 매체 복구를 수행할 수 있다.
...
이후 데이터베이스에서 **현재 시점의 로그 앵커**를 조회하여 복사된 체크포인트 이미지 파일의 **핑퐁 번호**를 일치시켜야 한다. 이때, 체크포인트 스케일에 따라 로그 앵커에서 확인해야 할 항목이 다르다는 점을 주의한다.
