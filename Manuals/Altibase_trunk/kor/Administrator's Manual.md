...
체크포인트 스케일을 싱글로 설정하면, 메모리 테이블스페이스를 생성할 때 하나의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행하면 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만, 결과적으로 안정적인 체크포인트 이미지 파일 하나만 남게 된다. 이는 디스크 사용량의 효율성을 높인다.
...
기본적으로 사용자는 파일의 이름에서 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알 수 있다. 그러나 파일이 유실되거나 손상되어 Altibase 서버에서 저장하고 있는 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 확인해야 한다면 아래의 두 가지 방법으로 조회할 수 있다.
...
- **풀 라이트**
  ...
  ① 불안정한 체크포인트 이미지 파일을 생성한다.

  ② 불안정한 체크포인트 이미지 파일에 더티 페이지가 포함된 체크포인트 이미지 파일의 내용 전부를 기록한다. 

  ③ WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다. 

  ④ 로그 앵커에서 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 ①에서 생성한 체크포인트 이미지 파일의 핑퐁 번호로 변경한다. 

  ⑤ 기존의 체크포인트 이미지 파일을 삭제한다.

- **더블 라이트**

  더티 페이지가 체크포인트 이미지 파일의 50% 이하일 때 기록하는 방식이다. 

  ① 더블 라이트 이미지 버퍼에 더티 페이지의 스냅샷을 저장한다. 
  
  ② 더블 라이트 이미지 파일에 ①에서 저장한 더티 페이지의 스냅샷을 기록한다. 
  
  ③ WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다.
  
  ④ 더블 라이트 이미지 버퍼에 저장한 더티 페이지의 스냅샷을 안정적인 체크포인트 이미지 파일에 직접 기록한다. 
...
**더블 라이트 리커버리**는 체크포인트 이미지 파일에 기록 중이던 더티 페이지를 메모리에 복구하는 동작으로, Altibase 서버 구동 시 META 단계에서 자동으로 수행된다.

서버가 더블 라이트 방식으로 체크포인트를 수행하던 도중 비정상 종료되면, 안정적인 체크포인트 이미지 파일이 손상될 수 있다. 이 때 서버를 다시 구동하면 Altibase는 체크포인트 이미지 파일의 손상을 감지하고 더블 라이트 리커버리를 수행하여 체크포인트 이미지 파일을 복원할 때 필요한 더티 페이지를 복구한다.

일반적으로 더블 라이트 리커버리는 시스템이 자동으로 수행한다. 그러나 예외적으로 매체 복구를 수행하거나 체크포인트 스케일 설정을 변경할 때 `ERR-111C6 : The process was interrupted while performing a Scale SINGLE Checkpoint.` 오류가 발생했다면, 사용자는 서버를 다시 구동하여 더블 라이트 리커버리를 수행할 수 있도록 조치를 취해야 한다. 

만약 서버를 구동할 수 없다면 이전에 백업한 체크포인트 이미지 파일을 사용하여 매체 복구를 수행해야 한다. 이에 대한 절차는 [매체복구 사례 7](#매체복구-사례-7)을 참고한다.

> [!important]
>
> 더블 라이트 리커버리를 수동으로 수행한 후에는 **반드시 체크포인트를 수행해야 한다.**
>
> 체크포인트 이미지 파일은 더블 라이트 리커버리로 복구된 더티 페이지를 체크포인트 이미지 파일에 기록해야만 완전히 복원된다. 기본적으로 서버 구동 시 체크포인트가 자동으로 수행되지만, [프로퍼티 설정](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-1.Data%20Types%20%26%20Altibase%20Properties.md#checkpoint_enabled)에 따라 체크포인트가 비활성화될 수 있기 때문에 되도록 체크포인트를 사용자가 직접 수행하는 것을 권장한다.
