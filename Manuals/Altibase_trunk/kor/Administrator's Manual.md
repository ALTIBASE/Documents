...
메모리 데이터베이스에서의 더블 라이트란 체크포인트 이미지를 관리하는 방식을 싱글로 설정했을 경우에 체크포인트 이미지를 갱신하는 방법 중 하나를 의미한다. 메모리용 더블 라이트 파일은 더블 라이트 동작 중에 사용하는 버퍼 파일을 의미한다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트)를 참조한다.
...
체크포인트 시 메모리 공간의 페이지들은 운영 체제에 의해 지원되는 파일에 저장된다. Altibase의 체크포인트 방식은 "핑퐁 체크포인트"이다. 핑퐁 체크포인트란 체크포인트 수행 시 핑퐁 번호 0번과 핑퐁 번호 1번 체크포인트 이미지 파일 두 개를 번갈아 가며 사용하는 것을 의미한다. 또한 각 체크포인트 이미지 파일은 디스크 입출력 비용의 분산을 목적으로 다수의 작은 파일로 분리될 수 있다.

> [!TIP]
>
> <a name="pingpongnumber"></a>핑퐁 번호란 체크포인트 이미지 파일에 자동으로 부여되는 번호로서, 0과 1을 사용한다. 핑퐁 번호는 체크포인트 이미지 파일의 이름에서 테이블스페이스 이름 뒤에 위치한 숫자로 확인할 수 있다. 체크포인트 이미지 파일의 이름 규칙은 다음과 같다.
>
> ````
> 테이블스페이스 이름-{핑퐁 번호}-{파일 번호}
> ````
>
> 예를 들어, `MY_MEM_TBS-0-0` 라는 체크포인트 이미지 파일의 이름에서 사용자는 이것이 MY_MEM_TBS라는 메모리 테이블스페이스의 체크포인트 이미지 파일이며, 핑퐁 번호 0번을 부여받았으며, 첫 번째로 생성된 파일인 것을 알 수 있다.
...
체크포인트 이미지 경로는 메모리 테이블스페이스에만 적용된다. Altibase는 메모리 테이블스페이스의 고성능 트랜잭션 처리를 위하여 핑퐁 체크포인트를 사용한다. 핑퐁 체크포인트를 위해서 두 벌의 체크포인트 이미지가 디스크에 생성된다. 각 체크포인트 이미지는 다수의 파일에 분할되어 저장될 수 있다. 체크포인트 이미지가 분할되는 크기는 SPLIT EACH절에 명시할 수 있다. 분할된 파일은 디스크 입출력 비용을 분산하기 위하여 서로 다른 경로에 저장될 수 있으며, 사용자가 임의로 분할의 크기 및 체크포인트 이미지 파일들이 저장되는 경로들을 지정할 수 있다. 사용자는 체크포인트 이미지 경로를 추가하거나 변경할 수 있지만, 한번 지정된 분할의 크기를 변경할 수는 없다.
...
핑퐁 체크포인트는 체크포인트 수행 시 체크포인트 이미지 파일에 [핑퐁 번호](#pingpongnumber) 0과 1을 부여하고, 더티 페이지를 핑퐁 번호 0번 체크포인트 이미지 파일과 핑퐁 번호 1번 체크포인트 이미지 파일에 번갈아 쓰는 것이다.
...
체크포인트 스케일은 체크포인트 이미지 파일을 관리하는 방식이다. 사용자는 데이터베이스를 생성할 때 체크포인트 스케일을 설정할 수 있고, 데이터베이스 생성 이후에는 Altibase 서버 구동 단계 중 CONTROL 단계에서 체크포인트 스케일을 변경할 수 있다. Altibase는 다음의 두 가지 체크포인트 스케일을 지원한다.

###### 체크포인트 스케일 페어(Checkpoint Scale Pair)

체크포인트가 동작하면 각 메모리 테이블스페이스마다 핑퐁 번호 0번과 1번을 부여받은 체크포인트 이미지 파일 두 개를 생성하고, 체크포인트를 수행 할 때마다 더티 페이지를 두 체크포인트 이미지 파일에 번갈아 기록한다. 데이터베이스 생성 시 체크포인트 스케일을 따로 설정하지 않았다면 핑퐁 체크포인트는 체크포인트 스케일 페어 방식으로 동작한다.

마지막으로 기록된 체크포인트 이미지 파일을 안정적인 체크포인트 이미지라고 한다. 안정적인 체크포인트 이미지의 핑퐁 번호는 테이블스페이스마다 다르기 때문에, 특정 테이블스페이스의 안정적인 체크포인트 이미지의 핑퐁 번호를 알고 싶다면 로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목을 살펴봐야 한다.

다음은 테이블스페이스 *MY_TBS*의 체크포인트 이미지 파일을 관리하는 예시이다.

| Altibase 동작 | 핑퐁 번호 0번 | 핑퐁 번호 1번 | 안정적인 체크포인트 이미지의 핑퐁 번호 |
| ------------------------------- | ------------------------------- | ------------------------------- | ------------------------------- |
| 최초 테이블스페이스 생성 | MY_TBS-0-0 생성 | MY_TBS-1-0 생성 | 0 |
| 체크포인트 수행 | MY_TBS-0-0                     | MY_TBS-1-0 갱신                   | 1                     |
| 체크포인트 수행             | MY_TBS-0-0 갱신                  | MY_TBS-1-0                     | 0                     |
| 체크포인트 수행              | MY_TBS-0-0         | MY_TBS-1-0,<br>MY_TBS-1-1 다음 파일번호 생성 | 1                     |

###### 체크포인스 스케일 싱글

체크포인트가 동작하면 각 메모리 테이블스페이스마다 안정적인 체크포인트 이미지 파일 한 개만 남기도록 한다. 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만 이 방식은 디스크 사용량의 효율성을 높일 수 있다.

안정적인 체크포인트 이미지의 핑퐁 번호는 체크포인트 이미지 파일마다 다르기 때문에, 특정 테이블스페이스의 안정적인 체크포인트 이미지의 핑퐁 번호를 알고 싶다면 로그 앵커의 "[ MEMORY CHECKPOINT IMAGE ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목을 살펴봐야 한다.

다음은 테이블스페이스 *MY_TBS*의 체크포인트 이미지 파일을 관리하는 예시이다. 다음과 같이 **안정적인 체크포인트 이미지 파일 1개를** 유지한다.

| Altibase  동작           | 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 | 안정적인 체크포인트 이미지 핑퐁 번호 |
| ------------------------ | ------------------------------- | ------------------------------- | ------------------------------------ |
| 최초 테이블스페이스 생성 | MY_TBS-0-0                      | (MY_TBS-1-0 삭제됨)             | 0                                    |
| 체크포인트 수행          | (MY_TBS-0-1 삭제됨)             | MY_TBS-1-1                      | 1                                    |
| 체크포인트 수행          | MY_TBS-0-2                      | (MY_TBS-1-2 삭제됨)             | 0                                    |
...
2. 메모리 테이블스페이스에 대해 핑퐁 체크 포인트 기법을 사용하고 있기 때문에, 백업 시에는 메모리 테이블스페이스의 안정적인 체크포인트 이미지 파일만 복사되었다.  
...
3. 백업된 메모리 테이블스페이스는 안정적인 체크포인트 이미지 파일이기 때문에 전부 복사한다.
...
먼저 Loganchor 헤더의 Checkpoint Scale 항목을 참고하여 체크포인트 방식을 구분한다.

Checkpoint Scale 항목의 값이 PAIR인 경우 테이블스페이스 속성의 Stable Checkpoint Image Num. 항목을 참고한다.

Checkpoint Scale 항목의 값이 SINGLE인 경우 메모리 체크포인트 이미지 속성의 Stable Single Checkpoint Image Num. 항목을 참고한다.
