...
[<sup>6</sup>] Altibase는 메모리 상에 존재하는 테이블스페이스의 데이터의 영속성(Durability) 보장을 위해 디스크의 파일에 데이터를 저장한다. 이와 같이 테이블스페이스의 데이터가 저장되는 파일을 체크포인트 이미지 파일이라고 한다. Altibase가 사용하는 핑퐁 체크포인트 방식은 두 벌의 체크포인트 이미지 파일을 두고, 하나씩 번갈아 가면서 테이블스페이스의 데이터가 저장된다.
...
다음은 체크포인트 스케일이 페어로 설정된 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지 파일이 어떻게 관리되는지 보여주는 예시이다.
...
위의 결과에서 사용자는 Altibase가 *MY_MEM_TBS*를 구성한 페이지마다 체크포인트 이미지 파일 한 쌍을 관리한다는 것을 알 수 있다. 즉, 체크포인트 이미지 파일 MY_MEM_TBS-0-0에 기록된 페이지는 MY_MEM_TBS-1-0에도 기록되어 있는 것이다.

페어 방식으로 체크포인트 이미지 파일을  관리하면 테이블스페이스마다 고유한 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 가진다. 따라서 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알고 싶다면 [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla)를 사용하여 로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목을 살펴봐야 한다.
...
다음은 체크포인트 스케일이 싱글로 설정된 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지 파일이 어떻게 관리되는지 보여주는 예시이다.
...
위의 결과에서 사용자는 Altibase가 다음과 같이 *MY_MEM_TBS*를 구성한 페이지마다 안정적인 체크포인트 이미지 파일을 하나만 유지하고 있는 것을 알 수 있다. 즉, MY_MEM_TBS-0-0에 기록된 페이지는 이 파일에만 존재하지만, 안정적인 체크포인트 이미지 파일이기 때문에 가장 최근의 변경 내용을 반영하고 있다는 뜻이다.

싱글 방식으로 체크포인트 이미지 파일을 관리하면 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 체크포인트 이미지 파일마다 다르다. 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 알고 싶다면 [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla)를 사용하여 로그 앵커의 "[ MEMORY CHECKPOINT IMAGE ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목을 살펴봐야 한다.
