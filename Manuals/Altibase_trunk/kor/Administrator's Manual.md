...
더블 라이트는 데이터 무결성을 유지하기 위해 손상 가능성이 있는 데이터를 복구할 수 있도록 페이지를 두 번 저장하는 동작이다. Altibase는 저장할 데이터가 속한 데이터베이스에 따라 두 종류의 더블 라이트 파일을 관리한다.
...
메모리 데이터베이스에서 체크포인트 스케일을 싱글로 설정하면, 체크포인트 이미지 파일을 갱신하는 동안 더블 라이트 동작이 수행될 수 있다. 이때 사용되는 버퍼 파일을 더블 라이트 이미지 파일이라고 한다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.

#### 퍼지&핑퐁 체크포인트 

Altibase는 최근의 데이터베이스 상태를 안전하게 백업(Backup) 데이터베이스로 반영하기 위해 퍼지&핑퐁 체크포인트를 수행한다.

메인 메모리 데이터베이스에서의 퍼지 체크포인트(Fuzzy checkpoint)를 수행하면 모든 변경된 데이터 페이지가 백업 데이터베이스로 내려가 현재 수행중인 트랜잭션의 수행에 영향을 미칠 수 있다. 따라서 퍼지 체크포인트와 더불어 핑퐁 체크포인트(Ping pong checkpoint)를 함께 수행한다. 핑퐁 체크포인트는 백업 데이터베이스를 두 개로 관리한다. 이는 체크포인트 과정에서 부하를 줄이고 트랙잭션 동작의 성능을 최대한 높인다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.
...
- SYS_DOUBLE_WRITE_IMAGE: 메모리 테이블스페이스의 더티 페이지를 체크포인트 이미지에 기록할 때 메모리 페이지를 임시로 저장하는 더블 라이트 이미지 파일
...
테이블에 대해 다음과 같은 SQL문이 지원된다. 이에 대한 자세한 설명은 *[SQL Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)*를 참조한다.
...
임시 테이블 생성은 CREATE [GLOBAL] TEMPORARY TABLE 구문을 사용한다. ON COMMIT 절은 테이블의 데이터가 트랜잭션에 한정되는지 또는 세션에 한정되는지를 지정한다. ON COMMIT 절에 대한 자세한 설명은 [*SQL Reference*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)를 참고하기 바란다.
...
임시 테이블에 대해 다음과 같은 SQL문이 지원된다. 이에 대한 자세한 설명은 *[SQL Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)*를 참조한다.
...
압축 테이블 생성은 일반 테이블과 동일하게 CREATE TABLE 구문을 사용한다. 다만 COMPRESS 절에 압축할 칼럼의 이름을 명시한다. COMPRESS 절에 대한 자세한 설명은 *[SQL Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)*를 참고하기 바란다.
...
압축 테이블에 대해 다음과 같은 SQL문이 지원된다. 이에 대한 자세한 설명은 *[SQL Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md#alter-table)*를 참조한다.
...
Materialized view에 대해 다음과 같은 SQL문을 제공하며, 이에 대한 자세한 설명은 *[SQL Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)*을 참조한다.
...
Materialized view를 위해 실제 데이터가 유지되는 테이블에 대해 다음과 같은 SQL문을 제공하며, 이에 대한 자세한 설명은 [*SQL Reference*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)을 참조한다.
...
-   작업(JOB)을 생성하기 전에 JOB_SCHEDULER_ENABLE, JOB_THREAD_COUNT 프로퍼티의값이 0이 아닌 값으로 설정되어 있어야 한다. 프로퍼티에 대한 자세한 설명은 General Reference를 참조하기 바란다.
...
생성된 JOB에 대한 정보는 SYS_JOBS\_ 메타 테이블에서 확인할 수 있다. 프로퍼티와 메타 테이블에 대한 상세한 설명은 [*General Reference-2.The Data Dictionary*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General%20Reference-2.The%20Data%20Dictionary.md)를 참조한다.
...
Altibase가 지원하는 전체 시스템 접근 권한 목록은 다음과 같다. 각 권한에 대한 자세한 설명은 [*SQL Reference*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/SQL%20Reference.md)을 참조한다.
...
메모리 테이블스페이스는 디스크 테이블스페이스와 달리 데이터를 데이터 파일에 저장하지 않고, 선형적인 메모리 공간에 저장한다. 선형적인 메모리 공간은 페이지 단위로 분할되고, 이 페이지들의 리스트가 테이블을 구성한다. 디스크 테이블스페이스는 디스크 입출력 비용 및 대용량 테이블 관리를 위하여 페이지 단위가 아닌 익스텐트 단위로 관리한다. 세그먼트는 개념적으로 익스텐트의 리스트를 관리하기 위한 논리적인 단위이다. 그러나 메모리 테이블스페이스의 목적은 대용량 데이터의 관리보다 빠른 접근을 지원하는 것이기 때문에, 세그먼트나 익스텐트의 개념이 필요하지 않다. 따라서 메모리 테이블스페이스의 테이블들은 페이지 리스트를 이용하여 관리된다.

체크포인트를 수행하면 메모리 테이블스페이스의 페이지에 기록된 메모리 테이블의 데이터는 물리적으로 체크포인트 이미지 파일에 저장된다. 체크포인트 이미지 파일은 객체들을 저장하기 위한 디스크 테이블스페이스의 데이터 파일과 달리 객체들을 백업하기 위해 사용된다. 따라서 체크포인트 이미지 파일이 데이터베이스 운영에 직접적으로 필요하지는 않지만, 백업 및 복구 시간을 단축하기 위해서는 반드시 필요하다.

Altibase는 메모리 데이터베이스에 핑퐁 체크포인트를 수행하여, 갱신된 체크포인트 이미지를 체크포인트 이미지 파일 두 개에 번갈아 기록하도록 한다. 체크포인트 이미지는 디스크 입출력 비용을 분산시키기 위해 다수의 작은 파일로 분할될 수 있다. 체크포인트에 관한 자세한 설명은 [8장 트랜잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.
...
메모리 테이블스페이스에 적용되는 속성은 디스크 테이블스페이스의 데이터 파일 속성과 유사하지만, 추가적으로 체크포인트 이미지 파일 경로를 포함한다. 구문은 다음과 같다.

```
SIZE {①초기 크기절} 
AUTOEXTEND [②자동확장절 
MAXSIZE [③최대크기절] ]
CHECKPOINT PATH [④체크포인트 이미지 파일 경로절]
```
...
```
CHECKPOINT PATH ‘체크포인트 이미지 파일 경로 리스트’ 
SPLIT EACH integer [K/M/G]
```

체크포인트 이미지 파일 경로는 메모리 테이블스페이스에만 적용된다. Altibase는 메모리 테이블스페이스의 고성능 트랜잭션 처리를 위하여 핑퐁 체크포인트를 사용한다. 핑퐁 체크포인트를 위해서 두 개의 체크포인트 이미지 파일이 디스크에 생성된다. 체크포인트 이미지는 다수의 파일에 분할되어 저장될 수 있고, 분할되는 크기는 SPLIT EACH 절에 명시할 수 있다. 분할된 파일은 디스크 입출력 비용을 분산하기 위하여 서로 다른 경로에 저장될 수 있으며, 사용자가 임의로 분할의 크기 및 체크포인트 이미지 파일들이 저장되는 경로들을 지정할 수 있다. 사용자는 체크포인트 이미지 경로를 추가하거나 변경할 수 있지만, 한번 지정된 분할의 크기를 변경할 수는 없다.

##### 휘발성 테이블스페이스 속성

휘발성 테이블스페이스에 적용되는 속성은 체크포인트 이미지 파일 경로를 제외하고는 메모리 테이블스페이스의 속성과 유사하다.
...
이는 메모리 시스템 테이블스페이스와 메모리 사용자 정의 테이블스페이스에만 사용될 수 있으며, 아래와 같은 옵션들을 가진다. 체크포인트 이미지 파일 경로에 대한 추가, 삭제 및 변경은 어느 구동 단계에서도 수행 가능하다. 그러나 서비스 단계에서는 오프라인 테이블스페이스만 변경이 가능하다.
...
기존 체크포인트 이미지 파일 경로를 새로운 경로로 변경한다.
...
이번 절에서는 테이블스페이스의 온라인/오프라인 백업의 개념 및 특징을 간략히 설명한다. 백업 및 복구에 대한 자세한 설명은 이 매뉴얼의 해당 장과 [*Getting Started Guide*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Getting%20Started%20Guide.md)을 참고한다.
...
또한 체크포인트 이미지 파일 경로는 기본적으로 MEM_DB_DIR 프로퍼티에 지정한 하나 혹은 그 이상의 체크포인트 이미지 파일 경로가 새로 생성된 테이블스페이스을 위한 체크포인트 이미지 파일 경로로 사용된다.

altibase.properties 에 아래의 예처럼 두 개의 체크포인트 이미지 파일 경로가 명시되어 있다고 가정하자. MEM_DB_DIR프로퍼티의 값이 Altibase 홈 디렉터리 아래의 dbs1와 dbs2, 두 개로 지정되어 있다.
...
이와 같은 경우에 다음의 쿼리를 사용해서 앞서 생성한 USER_MEM_TBS 테이블스페이스를 위한 체크포인트 이미지 파일 경로로 MEM_DB_DIR에 지정한 dbs1과 dbs2가 지정된 것을 확인할 수 있다.
...
우선, 체크포인트 이미지 파일 경로 안의 파일들을 살펴보자. dbs1 디렉터리 내에 다음과 같이 6개의 파일이 존재하는 것을 볼 수 있다.
...
이 파일들은 모두 메모리 테이블스페이스의 체크포인트 이미지 파일이다.

Altibase는 각 메모리 테이블스페이스의 체크포인트 이미지를 SPLIT EACH 절에 지정한 단위만큼 나누어 체크포인트 이미지 파일에 기록한다. 즉, 체크포인트 이미지의 크기가 지정한 단위보다 커지면 Altibase는 체크포인트 이미지를 여러 파일로 분할한다. 분할된 체크포인트 이미지 파일은 생성된 순서대로 0부터 1씩 증가하는 파일 번호를 부여받는다. 체크포인트 이미지 파일 이름에 관한 좀 더 자세한 설명은 [여기](#checkpointimagefilenaming)를 참고하라.

위의 CREATE MEMORY TABLESPACE 구문에서는 SPLIT EACH 절을 사용하지 않았기 때문에 DEFAULT_MEM_DB_FILE_SIZE 프로퍼티에 지정된 기본값인 1GB 단위로 체크포인트 이미지 파일들이 분할된다. 위 세 개의 테이블스페이스에 사용된 공간이 아직 1GB에 도달하지 않았기 때문에, 파일 번호는 0까지만 존재하는 것을 볼 수 있다.

다음은 dbs1 디렉터리 내에 존재하는 6개의 체크포인트 이미지 파일에 관한 설명이다.

- SYS_TBS_MEM_DATA: 기본 시스템 데이터 테이블스페이스이다. 사용자가 테이블스페이스를 명시하지 않고 테이블을 생성할 때, 테이블의 데이터는 이 테이블스페이스에 저장된다

- SYS_TBS_MEM_DIC: 메타 데이터를 지니는 시스템 딕셔너리 테이블스페이스이다. 이 테이블스페이스는 데이터베이스 생성 시 자동으로 만들어진다.

- USER_MEM_TBS: 앞서 생성한 사용자 정의 데이터 테이블스페이스이다.
...
다음과 같이 체크포인트 이미지 파일 경로를 명시하여 메모리 테이블스페이스를 생성할 수도 있다.
...
위의 예제처럼, 상대경로 dbs1을 체크포인트 이미지 파일 경로로 지정하는 것은 \$ALTIBASE_HOME/dbs1을 지정하는 것과 동일하다. 또한, CREATE TABLESPACE 구문에 지정한 체크포인트 이미지 파일 경로를 실제로 파일 시스템에 생성하고, 거기에 쓰기와 실행 권한을 주는 작업은 테이블스페이스를 생성하기 전에 DBA가 수동으로 해야 한다.
...
##### 메모리 테이블스페이스에 체크포인트 이미지 파일 경로 추가

이 절에서는 메모리 테이블스페이스에 체크포인트 이미지 파일 경로를 추가하는 절차에 대해 알아본다. 메모리 테이블스페이스를 위한 체크포인트 이미지 파일 경로는 컨트롤(CONTROL) 구동 단계에서만 설정이 가능하다. 우선 Altibase 서버를 종료 시킨 후에 다음과 같이 컨트롤 단계까지 Altibase를 기동한다.
...
앞서 생성한 USER_MEM_TBS 테이블스페이스를 위한 체크포인트 이미지 파일 경로를 조회하기 위해서는 V\$MEM_TABLESPACE_CHECKPOINT_PATHS 성능 뷰를 이용하면 된다.

만약 테이블스페이스의 데이터가 빈번하게 변경되어 체크포인트 시 디스크 I/O의 양이 증가했다면, 이는 다음과 같이 기존의 체크포인트 이미지 파일 경로와 물리적으로 다른 디스크를 경로로 추가해서 경감시킬 수 있다.

USER_MEM_TBS에 /new_disk/dbs3 경로를 새로 추가해보자. 우선, 추가하고자 하는 체크포인트 이미지 파일 경로와 디렉터리를 생성하고, Altibase 프로세스가 해당 디렉터리에 쓰기와 실행 권한을 가지도록 권한을 설정해야 한다.

Altibase 프로세스를 수행하는 운영체제의 사용자 계정이 altibase라고 가정한다.
...
이제 다음과 같이 ADD CHECKPOINT PATH구문을 이용하여 체크포인트 이미지 파일 경로를 추가할 수 있다.
...
기존의 체크포인트 이미지 파일 경로에 존재하는 체크포인트 이미지 파일들을 새로 추가된 체크포인트 이미지 파일 경로로 이동시키는 것은 DBA의 책임이다. Altibase는 체크포인트 이미지 파일 경로가 추가된 이후에 체크포인트가 발생하면, 새로운 체크포인트 이미지 파일을 생성할 때 추가된 체크포인트 이미지 파일 경로에 생성한다.<sup>7</sup>

[<sup>7</sup>] 체크포인트 시 테이블스페이스의 체크포인트 이미지 파일을 새로 생성할 경우, 테이블스페이스에 속한 각각의 모든 체크포인트 이미지 파일 경로를 하나씩 번갈아가면서 사용한다.

##### 메모리 테이블스페이스의 체크포인트 이미지 파일 경로 변경

이 절에서는 메모리 테이블스페이스의 체크포인트 이미지 파일 경로를 변경하는 절차에 대해 알아본다. 메모리 테이블스페이스를 위한 체크포인트는 컨트롤(CONTROL) 구동 단계에서만 설정이 가능하다. 앞서 ‘메모리 테이블스페이스에 체크포인트 이미지 파일 경로 추가’ 절에서 알아본 것과 같이 우선 Altibase 서버를 종료 시킨 후에 컨트롤 단계까지 Altibase를 기동한다.

본 예제는 기존의 체크포인트 이미지 파일 경로인 Altibase 홈 디렉터리 아래의 dbs1을 새로 설치한 디스크인 /new_disk로 옮기는 절차를 보여준다.

체크포인트 이미지 파일 경로를 변경하기 위해서는 기존의 경로를 절대경로로 정확히 입력해 주어야 한다. 컨트롤 단계에서 테이블스페이스의 체크포인트 이미지 파일 경로를 확인하는 방법은 위의 ‘메모리 테이블스페이스에 체크포인트 이미지 파일 경로 추가’절을 참고한다.

체크포인트 이미지 파일 경로를 변경할 때에는 추가할 때와 마찬가지로 다음과 같이 변경할 체크포인트 이미지 파일 경로와 디렉터리를 DBA가 직접 생성하고, 그 디렉터리에 대한 쓰기 및 실행 권한을 Altibase 프로세스를 실행하는 OS 사용자 계정에 부여해야 한다.
...
이제, 다음과 같이 RENAME CHECKPOINT PATH를 이용하여 Altibase 홈의 dbs1이라는 체크포인트 이미지 파일 경로를 새로 추가한 디스크인 /new_disk/dbs1으로 변경할 수 있다.
...
마지막으로 기존의 Altibase 홈 디렉터리의 dbs1안의 USER_MEM_TBS 테이블스페이스의 체크포인트 이미지 파일들을 모두 /new_disk/dbs1으로 옮겨주어야 한다.

```
$ mv $ALTIBASE_HOME/dbs1/USER_MEM_TBS* /new_disk/dbs1
```

##### 메모리 테이블스페이스의 체크포인트 이미지 파일 경로 제거

이 절에서는 메모리 테이블스페이스의 체크포인트 이미지 파일 경로를 제거하는 절차에 대해 알아본다. 메모리 테이블스페이스를 위한 체크포인트는 컨트롤(CONTROL) 구동 단계에서만 설정이 가능하다. 앞서 ‘메모리 테이블스페이스에 체크포인트 이미지 파일 경로 추가’ 절에서 알아본 것과 같이 우선 Altibase 서버를 종료 시킨 후 컨트롤 단계까지 Altibase를 기동한다.

본 예제는 기존의 체크포인트 이미지 파일 경로인 Altibase 홈 디렉터리의 dbs2를 제거하는 절차를 보여준다.

체크포인트 이미지 파일 경로를 변경하기 위해서는 기존의 경로를 절대경로로 정확히 입력해 주어야 한다. 컨트롤 단계에서 테이블스페이스에 속한 체크포인트 이미지 파일 경로를 보는 방법은 위의 ‘메모리 테이블스페이스에 체크포인트 이미지 파일 경로 추가’ 절을 참고한다

이제, 다음과 같이 DROP CHECKPOINT PATH 명령을 이용하여 Altibase 홈 디렉터리의 dbs2라는 체크포인트 이미지 파일 경로를 제거할 수 있다.
...
마지막으로 기존의 Altibase 홈 디렉터리의 dbs2에 있는 USER_MEM_TBS 테이블스페이스의 체크포인트 이미지 파일들을 USER_MEM_TBS의 다른 체크포인트 이미지 파일 경로 중 하나로 옮겨주어야 한다.
...
메모리 테이블스페이스는 디스크 테이블스페이스와 달리, 체크포인트 이미지 파일을 DBA가 직접 관리할 필요가 없다. 자동확장이 필요한 경우 Altibase가 체크포인트 이미지 파일을 자동으로 생성하기 때문이다.
...
다음 예제는 메모리 테이블스페이스 USER_MEM_TBS를 생성한 후, 체크포인트 이미지 파일을 삭제한 상태에서 해당 테이블스페이스만 제외한 나머지 테이블스페이스들로 Altibase를 기동하는 과정을 보여준다.
...
이제 Altibase를 종료하고, USER_MEM_TBS의 체크포인트 이미지 파일을 지운다. 이제 Altibase를 재기동하면 다음과 같은 에러가 발생한다.
...
그림에서 프리픽스드 인덱스는 sales_date를 키로 갖는다. 즉, 인덱스 파티션 키와 인덱스 키가 같은 칼럼 기반이다. 이러한 인덱스를 “프리픽스드 인덱스 (prefixed index)”라고 한다. 반면, 그림에서의 논프리픽스드 인덱스는 sales_id로 정렬되어 있다. 이러한 인덱스는 인덱스 파티션 키와는 다른 칼럼에 의해서 정렬된 인덱스로 “논프리픽스드 인덱스 (non-prefixed index)”라고 한다.
...
[그림 7-6]에서 인덱스 파티션 키는 sales_date칼럼이며 sales_id칼럼은 유니크 제약조건을 갖는다. sales_id칼럼을 인덱스 키로 갖는 논프리픽스드 인덱스(IDX_NON_PREFIX)가 구축된다면, 다음의 구문으로 레코드 삽입시 Altibase가 인덱스 IDX_NON_PREFIX를 이용해서 유니크 검사를 할 수 있을지 생각해보자:
...
인덱스는 테이블 파티션 키와 인덱스 파티션 키의 관계에 따라 글로벌 인덱스와 로컬 인덱스로 구분된다. “글로벌 인덱스 (global index)”는 인덱스 파티션 키가 테이블 파티션 키와 일치하지 않는 인덱스를 의미한다 (index_partition_key != table_partition_key). “로컬 인덱스 (local index)”는 인덱스 파티션 키가 테이블 파티션 키와 일치하는 인덱스를 의미한다 (index_partition_key == table_partition_key).![](media/Admin/7-7.png)
...
범위 파티션드 객체에 수행될 수 있는 연산의 종류는 5가지이다. 이 연산들은 파티션 분할, 파티션 삭제, 파티션 합병, 파티션 이름 변경, 파티션 레코드 삭제이다. 파티션 조건 변경은 현재는 지원되지 않는다.
...
- 인플레이스 분할(In-place Split)  
  기존 파티션의 레코드 일부를 잘라 새로운 파티션에 이동하는 분할 방식으로, 기존 파티션의 내용이 변경된다.  새로운 파티션의 이름이 기존 파티션의 이름과 같고, 새 파티션이 생성될 테이블스페이스를 지정하지 않으면 인플레이스 분할 방식이 사용된다. ([그림
  7-14] 참조)
...
트랜잭션의 수행 도중에 치명적인 오류가 있어 더 이상 진행할 수 없는 경우에는지금까지 수행해 왔던 모든 SQL 문들을 다시 되돌려서, 데이터베이스를 트랜잭션 수행 이전 상태로 바꿔야 한다. 이를 트랜잭션의 롤백이라고 한다.
...
디스크 테이블스페이스의 경우, 단시간에 다량의 갱신연산이 발생하면 데이터 테이블스페이스의 크기는 별 차이가 없지만, In-place MVCC의 영향으로 언두 로그 레코드의 양이 많아져서 언두 테이블스페이스의 크기가 증가하게 된다. 언두 테이블스페이스의 크기는 최초 CREATE DATABASE 시에 설정되어 변경될 수 없으므로 언두 로그 레코드의 재사용이 필요하다. 언두 로그 레코드는 트랜잭션이 커밋될 때
언두 테이블스페이스 헤더에 등록되어 연결 리스트(linked list)로 관리되다가, 현재 시스템 내의 모든 트랜잭션이 해당 언두 로그 레코드를 참조할 필요가 없어지게 되면 바로 해제 된다. 반면 트랜잭션이 롤백이 되면 언두 로그 레코드들은 언두 테이블스페이스에 등록되지 않고 바로 해제 된다.
...
체크포인트(checkpoint)는 주기적으로 메모리 데이터베이스의 내용을 백업 데이터 파일에 저장하는 것을 말한다. 체크포인트 수행은 시스템 장애로부터 데이터베이스 복구에 걸리는 시간을 줄이는데 그 목적이 있다.
...
퍼지 체크포인트는 체크포인트 진행 중에 다른 트랜잭션들의 수행을 허용하는 것이다. 퍼지 체크포인트를 수행하면 체크포인트 이미지 파일에는 커밋 또는 커밋되지 않은 트랜잭션들의 데이터가 혼재할 수 있다. "퍼지 체크포인트"는 이런 상태에서 유래된 이름이다.

핑퐁 체크포인트는 체크포인트를 수행할 때 더티 페이지를 두 개의 체크포인트 이미지 파일에 번갈아 기록하게 한다. 이 때 더티 페이지의 내용이 모두 반영된, 마지막으로 기록된 체크포인트 이미지 파일을 안정적인 체크포인트 이미지 파일이라고 한다.
...
하지만 Altibase는 체크포인트 대상 페이지에 래치를 획득하지 않고 체크포인트를 수행하여 성능 저하를 해결하고 있다. 이 때 체크포인트 도중 WAL 프로토콜을 지키지 않아서 발생할 수 있는 체크포인트 이미지 파일의 일관성 위배는 두 개의 체크포인트 이미지 파일에 번갈아 기록하여 해결한다. 

예를 들어 마지막으로 수행한 체크포인트 이미지 파일의 일관성이 깨진 상태로 Altibase 서버에 장애가 발생한 경우, 안정적인 체크포인트 이미지 파일을 사용해서 복구가 가능하다.

Altibase 7.4부터 체크포인트를 수행할 때 체크포인트 이미지를 관리하는 방식인 체크포인트 스케일(Checkpoint Scale)을 설정할 수 있다.

> [!NOTE]
>
> **체크포인트 이미지 파일 이름 규칙**<a name="checkpointimagefilenaming"></a>
>
> 체크포인트 이미지 파일의 이름 규칙은 다음과 같다.
>
> ```
> 테이블스페이스 이름-{핑퐁 번호}-{파일 번호}
> ```
>
> 핑퐁 번호는 0또는 1로, 핑퐁 체크포인트를 수행할 때 번갈아 사용하는 두 개의 체크포인트 이미지 파일을 구분하기 위한 번호를 의미한다. 핑퐁 번호로 메모리 데이터베이스를 백업하거나 복구할 때 사용하는 안정적인 체크포인트 이미지 파일을 식별할 수 있다.
>
> 파일 번호는 체크포인트 이미지가 기록된 체크포인트 이미지 파일의 생성 순서를 나타내는 번호로, 0부터 시작하여 1씩 증가한다. 체크포인트 이미지의 크기가 지정한 단위보다 커져 새로운 체크포인트 이미지 파일이 생성될 때 순차적으로 부여된다.
>
> 예를 들어, `MY_MEM_TBS-1-0` 라는 체크포인트 이미지 파일의 이름에서 사용자는 이 파일이 *MY_MEM_TBS*라는 메모리 테이블스페이스의 체크포인트 이미지 파일이고, 핑퐁 번호는 1번이며, 첫 번째로 생성된 체크포인트 이미지 파일인 것을 알 수 있다 .

##### 체크포인트 스케일

체크포인트 스케일은 체크포인트 이미지 파일을 생성, 갱신 및 유지하는 관리 방식이다. 사용자는 데이터베이스를 생성할 때 체크포인트 스케일을 설정할 수 있고, 데이터베이스 생성 이후에는 Altibase 서버 구동 단계 중 CONTROL 단계에서 체크포인트 스케일을 변경할 수 있다.

Altibase는 다음의 두 가지 체크포인트 스케일을 지원한다.

###### 체크포인트 스케일 페어(Checkpoint Scale Pair)

체크포인트 스케일을 페어(PAIR)로 설정하면, 메모리 테이블스페이스를 생성할 때 핑퐁 번호 0번과 1번을 부여받은 두 개의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행 할 때마다 더티 페이지를 두 개의 체크포인트 이미지 파일에 번갈아 기록한다. 

페어는 체크포인트 스케일의 기본값으로, 데이터베이스를 생성할 때 별도로 지정하지 않으면 페어로 설정된다.

다음은 체크포인트 스케일을 페어로 설정한 데이터베이스의 메모리 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지가 파일 3개로 분할되었을 때 체크포인트 이미지 파일이 어떻게 구성되는지 보여주는 예시이다.

```bash
# dbs 디렉터리에서 메모리 테이블스페이스 MY_MEM_TBS의 체크포인트 이미지 파일을 조회한 결과이다.
...
위의 결과에서 파일 번호마다 체크포인트 이미지 파일 한 쌍이 관리되고 있는 것을 알 수 있다.

|  | 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 |
| ------------------------------- | ------------------------------- | ------------------------------- |
| **파일 번호 0번** | MY_MEM_TBS-0-0 | MY_MEM_TBS-1-0 |
| **파일 번호 1번** | MY_MEM_TBS-0-1       | MY_MEM_TBS-1-1     |
| **파일 번호 2번** | MY_MEM_TBS-0-2    | MY_MEM_TBS-1-2      |

체크포인트 스케일을 페어로 설정하면 테이블스페이스마다 안정적인 체크포인트 이미지 파일의 핑퐁 번호가 다를 수 있다. 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 아래와 같은 방법으로 확인할 수 있다.

- [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla) 유틸리티를 사용하여 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목 값 조회
- [V$MEM_TABLESPACES](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_tablespaces) 혹은 [V$MEM_STABLE](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_table) 성능 뷰의 CURRENT_DB 값 조회

###### 체크포인트 스케일 싱글(Checkpoint Scale Single)

체크포인트 스케일을 싱글(SINGLE)로 설정하면, 메모리 테이블스페이스 생성할 때 하나의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행하면 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만, 결과적으로 안정적인 체크포인트 이미지 파일 하나만 남게 된다. 이는 디스크 사용량의 효율성을 높일 수 있다.

다음은 체크포인트 스케일을 싱글로 설정한 데이터베이스의 메모리 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지가 파일 3개로 분할되었을 때 체크포인트 이미지 파일이 어떻게 구성되는지 보여주는 예시이다.

```bash
#dbs 디렉터리에서 MY_MEM_TBS 메모리 테이블스페이스의 체크포인트 이미지 파일을 조회한 결과이다.
% ls -l | grep MY_MEM_TBS
rw-r----- 1 altibase altibase 323231744  7월 16 13:28 MY_MEM_TBS-0-0
rw-r----- 1 altibase altibase 323231744  7월 16 13:28 MY_MEM_TBS-1-1
rw-r----- 1 altibase altibase 323231744  7월 16 13:28 MY_MEM_TBS-0-2
...
위의 결과에서 *MY_MEM_TBS*를 구성하는 체크포인트 이미지 파일은 안정적인 체크포인트 이미지 파일 하나만 유지하고 있는 것을 알 수 있다. 즉, 각각의 파일은 모두 안정적인 체크포인트 이미지 파일이며 가장 최근의 변경 내용을 반영하고 있다.

|                   | 핑퐁 0번 체크포인트 이미지 파일 | 핑퐁 1번 체크포인트 이미지 파일 |
| ----------------- | ------------------------------- | ------------------------------- |
| **파일 번호 0번** | MY_MEM_TBS-0-0                  | -                               |
| **파일 번호 1번** | -                               | MY_MEM_TBS-1-1                  |
| **파일 번호 2번** | MY_MEM_TBS-0-2                  | -                               |

체크포인트 스케일을 싱글로 설정하면 안정적인 체크포인트 이미지 파일만 유지된다. 따라서 사용자는 파일의 이름을 통해 현재 안정적인 체크포인트 이미지의 핑퐁 번호를 알 수 있다.

그러나 [매체 복구](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Administrator's Manual.md#증분-백업에-대한-매체-복구)와 복구에 같이 과거 시점에 백업된 체크포인트 이미지를 사용하는 경우에는 현재 로그 앵커에 기록된 안정적인 체크포인트 이미지의 핑퐁 번호와 과거의 안정적인 체크포인트 이미지의 핑퐁 번호가 일치하지 않을 수 있다.

이럴 경우 특정 테이블스페이스의 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 아래와 같은 방법으로 확인할 수 있다.

- [dumpla](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Utilities%20Manual.md#dumpla) 유틸리티를 사용하여 "[ MEMORY CHECKPOINT IMAGE ATTRIBUTE ]" 영역의 "Stable Single Checkpoint Image Num." 항목 값 확인
- [V$MEM_STABLE](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md#vmem_table) 성능 뷰의 CURRENT_DB 값 확인

**체크포인트 스케일 싱글에서 체크포인트 이미지 파일 기록 방식**

체크포인트 스케일을 싱글로 설정했을 경우, 체크포인트 이미지 파일에서 갱신되어야 할 페이지의 비율에 따라 기록하는 방법이 달라진다. 더티 페이지가 체크포인트 이미지 파일의 절반을 초과하는 경우 풀 라이트(Full Write) 방식을, 그렇지 않을 경우 더블 라이트(Double Write) 방식을 선택한다. 

- 풀 라이트: 더티 페이지가 체크포인트 이미지 파일의 50%을 초과하는 경우의 기록 방식이다.

  1. 불안정한 체크포인트 이미지 파일을 새로 생성한다. 
  2. 새로 생성한 체크포인트 이미지 파일에 더티 페이지가 반영된 기존의 체크포인트 이미지 파일의 내용을 전부 기록한다. 
  3. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다. 
  4. 로그 앵커에서 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 1번에서 생성한 체크포인트 이미지 파일의 핑퐁 번호로 변경한다. 
  5. 기존의 체크포인트 이미지 파일을 삭제한다. 

- 더블 라이트: 더티 페이지가 체크포인트 이미지 파일의 50% 이하인 경우의 기록 방식이다. 

  1. 메모리용 더블 라이트 이미지 버퍼에 더티 페이지의 스냅샷을 저장한다. 

  2. 1에서 저장한 스냅샷을 더블 라이트 이미지 파일에 기록한다. 

  3. WAL 프로토콜을 구현하기 위하여 로그 레코드를 기록한다. 

  4. 메모리용 더블 라이트 이미지 버퍼에 저장한 스냅샷을 안정적인 체크포인트 이미지 파일에 직접 기록한다. 

     > [!CAUTION]
     >
     > 핑퐁 체크포인트의 더블 라이트 방식에서 사용하는 더블 라이트 이미지 버퍼 혹은 파일을 디스크 데이터베이스를 복구 할 때 사용하는 더블 라이트 파일과 혼동하지 않도록 주의한다.

     > [!NOTE]
     >
     > **더블 라이트 리커버리(Double Write Recovery)**
     >
     > 더블 라이트 리커버리는 손상되었을 가능성이 있는 페이지를 더블 라이트 이미지 파일을 이용하여 자동으로 복구하는 동작이다.
     >
     > 핑퐁 체크포인트 시 더블 라이트 방식을 사용할 경우, Altibase는 더티 페이지를 안정적인 체크포인트 이미지 파일에 직접 기록한다. 이 때 장애가 발생하면 부분 쓰기(Partial write)로 페이지가 손상될 수 있다. 이와 같은 경우 Altibase는 META 구동 단계에서 더블 라이트 리커버리가 자동으로 실행되어 수행하여 손상 가능성이 있는 페이지를 복구한다.
     >
     > 하지만 더블 라이트 리커버리는 페이지 복구에만 국한된다. 손상된 체크포인트 이미지 파일을 완전히 복구하려면, 더블 라이트 리커버리 후 체크포인트를 발생시켜 복구된 페이지를 체크포인트 이미지 파일에 기록해야 한다. 즉, 손상된 체크포인트 이미지 파일을 복구하는 과정은 다음과 같다.
     >
     > 1. 장애가 발생한 서버를 구동한다. 이 때 META 구동 단계에서 자동으로 더블 라이트 리커버리가 수행되어 페이지가 복구된다.
     > 2. 체크포인트를 수행하여 복구된 페이지를 체크포인트 이미지 파일에 기록한다.
     >
     > 체크포인트를 명시적으로 수행하는 대표적인 구문은 다음과 같다.
     >
     > ```sql
     > # 사용자가 임의로 체크포인트를 발생시키는 구문이다
     > iSQL> ALTER SYSTEM CHECKPOINT;
     > 
     > # 서버를 NORMAL 또는 IMMEDIATE 옵션으로 종료하면 자동으로 체크포인트를 수행한다
...
| [CHECKPOINT-SCALE] PAIR                               | 체크포인트 스케일 설정 확인                                  |
...
체크포인트 스케일이 페어로 설정되어 있을 경우, 메모리 테이블스페이스마다 핑퐁 번호 0번과 1번 체크포인트 이미지 파일이 모두 존재해야 Altibase가 정상적으로 운용된다. 따라서 안정적인 체크포인트 이미지 파일을 백업한 후 백업한 파일을 복사하는 과정이 필요하다.

로그 앵커의 "[ TABLESPACE ATTRIBUTE ]" 영역의 "Stable Checkpoint Image Num." 항목으로 안정적인 체크포인트 이미지 파일의 핑퐁 번호를 확인하고 그 번호의 파일만 백업하면 백업 수행 시간을 줄일 수 있다.

예를 들어 SYS_TBS_MEM_DIC 메모리 테이블스페이스의  "Stable Checkpoint Image Num." 항목 값이 1이라면 백업 받을 체크포인트 이미지 파일은 다음과 같다:
...
복구 작업을 위해서는 백업 받은 체크포인트 이미지 파일을 다음과 같이 복사해야 한다.
...
복사 후 매체 복구를 수행하면, 복구 완료 시점에 자동으로 안정적인 체크포인트 이미지 파일을 복사해서 불안정한(unstable) 체크포인트 이미지 파일을 생성한다.

체크포인트 스케일이 싱글로 설정되어 있을 경우, Altibase는 가장 최근에 체크포인트가 수행된 1개의 체크포인트 이미지 파일만 유지한다. 따라서 선별 과정 없이 그 체크포인트 이미지 파일을 사용하여 매체 복구를 수행한다.
...
1. 백업 받은 데이터 파일들을 원래 위치로 복사한다.

```
$ cp /backup_dir/*.dbf $ALTIBASE_HOME/dbs;
```

2. 메모리 테이블스페이스에 대해 핑퐁 체크포인트를 수행하기 때문에, 백업 시에는 안정적인 체크포인트 이미지 파일만 복사되었다.  
   예) 백업받은 메모리 테이블스페이스의 체크포인트 이미지 파일이 다음과 같다.
...
3. 백업된 안정적인 체크포인트 이미지 파일을 전부 복사한다.
...
위 결과는 백업된 데이터 파일을 사용하여 데이터베이스를 복원하기 위해서는 logfile4 아카이브 로그 파일 이후의 로그 파일들이 필요함을 나타낸다.

백업받은 안정적인 체크포인트 이미지 파일은 원위치에 복사해야 한다. 백업된 파일이 SYS_TBS_MEM_DIC-0-0이라면 아래와 같이 복사한다.

```
$ cp /backup_dir/SYS_TBS_MEM_DIC-0-0 $ALTIBASE_HOME/dbs;
```

체크포인트 이미지 파일의 핑퐁 번호를 확인하기 위해서는 \$ALTIBASE_HOME/bin/dumpla loganchor0 결과 중 loganchor 헤더와 SYS_TBS_MEM_DIC 테이블스페이스의 테이블스페이스 속성, 메모리 체크포인트 이미지 속성을 확인해야 한다.
...
백업한 체크포인트 이미지 파일의 핑퐁 번호는 [0]이고 현재 안정적인 체크포인트 이미지 파일의 핑퐁 번호는 [1]이므로, 다음과 같이 복사한다.
...
- 클라이언트  
  IPv6 를 사용해서 접속하려면, DSN 속성을 IPv6 주소로 지정하거나, 또는 DSN 속성은 호스트 이름으로 명시하고 PREFER_IPV6 속성을 TRUE로 지정하면 된다. 호스트 이름을 지정할 경우, Altibase 클라이언트는 getaddrinfo() 호출로 반환되는 모든 IP 주소로의 접속을 연결이 성공할 때까지 시도한다. 한 개 이상의 IP 주소가 반환될 경우, Altibase 클라이언트는 PREFER_IPV6 속성에 의한 순서대로 각 IP 주소로의 연결을 시도한다. PREFER_IPV6 속성이 지정되지 않거나 FALSE로 지정한 경우, 먼저 IPv4 주소로 연결을 시도한다. 연결이 실패하면 클라이언트는 반환되었던 IPv6 주소로 접속을 시도할 것이다. PREFER_IPV6 속성을 TRUE로 지정하면, IPv6 주소로 먼저 접속을 시도한다. 이것이 실패하면 클라이언트는 반환되었던 IPv4 주소로의 접속을 시도한다. PREFER_IPV6 속성에 대한 자세한 설명은 *[CLI User's Manual](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/CLI%20User's%20Manual.md)*를 참고하기 바란다.
...
더 자세한 설명은 [*CLI User's Manual*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/CLI%20User's%20Manual.md) 와 각각의 유틸리티에 대한 매뉴얼을 참고하기 바란다.
...
- altibase.properties 파일에서 관련 서버 프로퍼티를 설정한다. [*General Reference-1.Data Types & Altibase Properties \> 세션 관련 프로퍼티*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General%20Reference-1.Data%20Types%20%26%20Altibase%20Properties.md#%EC%84%B8%EC%85%98-%EA%B4%80%EB%A0%A8-%ED%94%84%EB%A1%9C%ED%8D%BC%ED%8B%B0)를 참고하기 바란다.
- ODBC/CLI 응용 프로그램에서는 CONNTYPE 속성을 지정하고, iSQL과 iLoader 같은 Altibase 유틸리티에서는 ISQL_CONNECTION 환경 변수를 설정한다. 자세한 설명은 [*CLI User's Manual*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/CLI%20User's%20Manual.md)과 각각의 유틸리티 매뉴얼을 참고하기 바란다.
...
각 프로퍼티에 대한 상세한 설명은 *[General Reference-1.Data Types & Altibase Properties](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-1.Data%20Types%20%26%20Altibase%20Properties.md)*를 참고하도록 한다.
...
- 아카이브 로그 파일 경로 (ARCHIVE_DIR 프로퍼티)  
  데이터베이스가 아카이브로그 모드로 운영중이라면, LOG_DIR 프로퍼티에 지정된 디렉터리에 위치하는 기록이 완료된 로그파일들은 ARCHIVE_DIR 프로퍼티에 지정된 디렉터리로 복사된다. 이러한 아카이브 로그파일은 데이터베이스 복구와 백업을 위해 사용된다. 이 프로퍼티의 수는 LOG_DIR프로퍼티의 수와 정확히 일치해야 한다. 아울러, LOG_DIR 프로퍼티가 여러 개인 경우 ARCHIVE_DIR 프로퍼티들은 LOG_DIR 프로퍼티 값과 순서대로 1:1로 매핑되도록 기술해야 한다.
...
- 로그파일 준비 쓰레드 (Log File Prepare Thread)  
  ...
  이 쓰레드는 로그 기록을 계속하기 위한 새 로그파일이 필요한 시점 전에 로그파일을 미리 만들어 둔다. 참고로, 체크포인트 중에 더 이상 사용되지 않는 로그파일들을 별도로 분류하여 체크포인트가 완료되는 시점에 이를 삭제한다.
- 로그 플러시 쓰레드 (Log Flush Thread)  
  최근에 기록된 로그를 주기적으로 디스크로 기록하는 쓰레드이다. 트랜잭션이 커밋될 때, Altibase 트랜잭션은 이 쓰레드가 관련 로그를 벌써 디스크에 쓰지 않았는지 검사하고, 기록되지 않은 로그만 디스크에 쓰게 된다. 즉, 이 쓰레드는 주기적으로 로그를 디스크에 기록함으로써 트랜잭션이 커밋되는 시점에 디스크에 기록할 로그의 수를 줄여주게 된다.
...
- UPDATE_TX_COUNT  
  데이터베이스에 변경을 가한 트랜잭션의 개수를 실시간으로 보여주는 통계치이다. 이 값이LFG_GROUP_COMMIT_UPDATE_TX_COUNT 프로퍼티에 설정된 값보다 클 때에만 그룹 커밋을 실시한다.
- GC_WAIT_COUNT  
...
- LFG_GROUP_COMMIT_UPDATE_TX_COUNT  
  이 프로퍼티의 값이 너무 작으면 데이터베이스에 변경을 가한 트랜잭션 수가 그다지 많지 않아도 그룹 커밋이 활성화된다. 이 경우, 로그를 기록하기 위한 디스크 I/O의 횟수가 증가하여 시스템 성능이 저하된다. 반면, 이 프로퍼티의 값이 너무 크면 비록 데이터베이스에 변경을 가한 트랜잭션의 수가 충분히 많더라도 그룹커밋이 활성화되지 못한다. 데이터베이스 시스템에 여러개의 트랜잭션이 몰리는 때에 DBA는 V\$LFG 성능 뷰의 UPDATE_TX_COUNT 값을 실시간으로 모니터링 하여 이 프로퍼티의 값을 적정한 값으로 결정하면 된다.
- LFG_GROUP_COMMIT_INTERVAL_USEC  
  이 프로퍼티의 값이 너무 작으면 빈번하게 디스크 I/O가 수행되어 시스템 성능이 저하된다. 이러한 상황에서는 커밋을 위해 실제로 디스크 I/O를 수행하는 횟수가 많아지므로 V\$LFG 성능 뷰의 GC_REAL_SYNC_COUNT 값이 빠른 속도로 증가한다. 반대로, 이 프로퍼티의 값이 너무 크면 이용 가능한 디스크 I/O 용량이 충분히 활용하지 못하여 시스템의 성능이 저하된다. 이러한 상황에서는 디스크 I/O를 위해 대기하는 횟수가 많아지므로 V\$LFG 성능 뷰의 GC_WAIT_COUNT 값이 빠른
  속도로 증가한다.  
  이 프로퍼티를 최적으로 설정하려면 우선 이 프로퍼티의 값을 기본값인 1000 (1ms)에서부터 시작하여 2배씩 증가시켜가면서 시스템의 전체 성능 (TPS - Transaction Per Second)을 측정해 본다. GC_REAL_SYNC_COUNT의 값이 점점 작아지면서 최적의 성능을 낼 때 이 프로퍼티가 최적화 됐음을 의미한다. GC_REAL_SYNC_COUNT의 값이 작아지는 것이 곧 최적의 성능을 내는 상황을 의미하지는 않는다.  
...
Altibase 데이터베이스의 운영 상태를 확인하기 위해서 메타 테이블과 성능 뷰를 이용할 수 있다. 메타 테이블과 성능 뷰에 대한 자세한 설명은 [*General Reference-2.The Data Dictionary*](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General_Reference-2.The%20Data%20Dictionary.md)를 참고한다.
...
- 시스템 리소스 부족  
  시스템 리소스 부족으로 인해 시스템 구동이 실패했다면, 여러 가지 시스템 리소스 중 어떤 리소스가 부족한지를 확인하여 실제 시스템에 적재되어 있는 리소스 가용량을 확인하고 시스템 커널 설정을 확인하여 문제가 있는 부분을
  해결해야 한다.  
  구동 시 주로 문제가 되는 리소스는 메모리 또는 세마포어이다. 메모리 관련 문제의 경우 시스템 커널 설정에서 한 프로세스 당 사용 가능한 메모리의 크기 및 세그먼트의 최대 크기 등을 확인해 봐야 한다. IPC 통신을 하려면 세마포어가 필요하며 이는 Altibase 프로퍼티들 중 IPC_CHANNEL_COUNT와 관련이 있다.
...
메모리 테이블스페이스의 저장 공간은 체크포인트가 수행될 때 실제로 디스크에 기록이 되기 때문에 메모리 상에 존재하는 테이블스페이스 크기 이상의 디스크 여유 공간이 필요하다.
...
위의 두 경우에 해당하지 않는다면 록을 기다리고 있는 상태를 의심할 수 있다. 현재 락 정보를 V\$LOCK 과 V\$LOCK_WAIT 성능 뷰로 확인하고 특정 세션이 불필요하게 록을 획득하고 있는 상태로 지속되고 있다면 해당 세션을 강제로 종료하여 해결할 수 있다.
...
프로퍼티 파일에 명시된 값은 ALTER SYSTEM 문을 이용하여 변경할 수 있다. 프로퍼티에 대한 자세한 설명은 *[General Reference](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/General%20Reference-1.Data%20Types%20%26%20Altibase%20Properties.md#trclog_detail_predicate)*를 참고하기 바란다.
