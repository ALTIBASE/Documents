...
메모리 데이터베이스에서의 더블 라이트는 체크포인트 스케일이 싱글일 때 더티 페이지가 적을 경우 이를 효율적으로 기록할 수 있도록 하는 동작이다. 메모리 데이터베이스에서 더블 라이트가 동작할 때 WAL(Write-Ahead Logging) 프로토콜을 준수하고 부분 쓰기(Partial Write)를 방지하기 위해 사용하는 파일을 더블 라이트 이미지 파일이라고 한다. 자세한 설명은 8장 트랙잭션 관리의 [메모리 DB의 체크포인트](#메모리-db의-체크포인트) 절을 참조한다.
...
- SYS_DOUBLE_WRITE_IMAGE: 메모리 테이블스페이스의 더티 페이지를 체크포인트 이미지에 기록하는 과정에서 더티 페이지를 임시로 저장하는 더블 라이트 이미지 파일
...
Altibase의 체크포인트 스케일 설정은 페어(Pair)와 싱글(Single)이 있다.
...
Altibase 7.4부터 지원하는 체크포인트 스케일 설정이다. 체크포인트 스케일을 싱글(SINGLE)로 설정하면, 메모리 테이블스페이스 생성할 때 하나의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행하면 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만, 결과적으로 안정적인 체크포인트 이미지 파일 하나만 남게 된다. 이는 디스크 사용량의 효율성을 높인다.
