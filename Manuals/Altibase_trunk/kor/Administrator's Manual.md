...
메모리 데이터베이스에서의 더블 라이트는 체크포인트 스케일이 싱글이고 더티 페이지의 양이 적을 때 체크포인트를 수행하는 방식을 의미한다. 이 과정에서 WAL(Write-Ahead Logging) 프로토콜을 준수하고 부분 쓰기(Partial Write)를 방지하기 위해 먼저 더티 페이지의 스냅샷을 더블 라이트 이미지 파일에 임시로 저장한 후 안정적인 체크포인트 이미지 파일에 기록한다. 자세한 설명은 8장 트랙잭션 관리의 [메모리 DB의 체크포인트](#메모리-db의-체크포인트) 절을 참조한다.

#### 퍼지&핑퐁 체크포인트 

Altibase는 최근의 데이터베이스 상태를 안전하게 백업(Backup) 데이터베이스로 반영하기 위해 퍼지&핑퐁 체크포인트를 수행한다.

메인 메모리 데이터베이스에서 퍼지 체크포인트(Fuzzy checkpoint) 방식만 사용하면 모든 변경된 데이터 페이지가 백업 데이터베이스로 내려가 현재 수행 중인 트랜잭션의 수행에 영향을 미칠 수 있다. 따라서 퍼지 체크포인트와 더불어 핑퐁 체크포인트(Ping-pong checkpoint) 방식을 함께 수행한다. 핑퐁 체크포인트는 백업 데이터베이스를 두 개로 관리한다. 이는 체크포인트 과정에서 부하를 줄이고 트랜잭션 동작의 성능을 최대한 높인다. 자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.자세한 설명은 [8장 트랙잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.
...
메모리 테이블의 데이터는 체크포인트 수행 시 물리적으로 체크포인트 이미지 파일에 저장된다. 디스크 데이터베이스의 데이터 파일은 객체 저장이 주요 목적이지만, 메모리 데이터베이스의 체크포인트 이미지 파일은 객체 백업을 위해 사용된다. 즉, 체크포인트 이미지 파일이 데이터베이스 운영에 직접적으로 필요하지는 않지만 재시작 및 백업/복구 시간을 단축하기 위해서는 반드시 필요하다.

Altibase는 메모리 데이터베이스에 핑퐁 체크포인트를 수행하여 더티 페이지를 체크포인트 이미지 파일 두 개에 번갈아 기록한다. 체크포인트 이미지는 디스크 입출력 비용을 분산하기 위해 여러 개의 작은 파일로 나뉠 수 있다. 체크포인트에 관한 자세한 설명은 [8장 트랜잭션 관리의 체크포인트](#체크포인트) 절을 참조한다.
...
Altibase의 체크포인트 스케일 설정은 페어(Pair)와 싱글(Single)이 있다. 페어는 Altibase의 전통적인 체크포인트 이미지 파일 관리 방식이며, 싱글은 Altibase 7.4부터 지원하는 설정이다.

###### 체크포인트 스케일 페어(Checkpoint Scale Pair)

체크포인트 스케일을 페어로 설정하면, 메모리 테이블스페이스를 생성할 때 핑퐁 번호 0번과 1번을 부여받은 두 개의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행 할 때마다 더티 페이지를 두 개의 체크포인트 이미지 파일에 번갈아 기록한다. 

페어는 체크포인트 스케일의 기본값으로, 데이터베이스를 생성할 때 별도로 지정하지 않으면 페어로 설정된다.

다음은 체크포인트 스케일을 페어로 설정한 데이터베이스에서, 메모리 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지가 파일 3개로 나누어졌을 때 체크포인트 이미지 파일이 어떻게 구성되는지 보여주는 예시이다.

dbs 디렉터리에서 메모리 테이블스페이스 MY_MEM_TBS의 체크포인트 이미지 파일을 조회하였다.
...
체크포인트 스케일을 싱글로 설정하면, 메모리 테이블스페이스 생성할 때 하나의 체크포인트 이미지 파일이 만들어진다. 이후 체크포인트를 수행하면 내부적으로는 핑퐁 체크포인트 방식으로 동작하지만, 결과적으로 안정적인 체크포인트 이미지 파일 하나만 남게 된다. 이는 디스크 사용량의 효율성을 높인다.

다음은 체크포인트 스케일을 싱글로 설정한 데이터베이스에서, 메모리 테이블스페이스 *MY_MEM_TBS*의 체크포인트 이미지가 파일 3개로 나누어졌을 때 체크포인트 이미지 파일이 어떻게 구성되는지 보여주는 예시이다.
...
사용자는 파일의 이름을 통해 현재 데이터베이스의 안정적인 체크포인트 이미지의 핑퐁 번호를 알 수 있다. 그러나 매체 복구를 수행해야 상황에서 과거에 백업된 체크포인트 이미지 파일 또는 로그 앵커를 사용하는 경우에는 현재 로그 앵커에 기록된 안정적인 체크포인트 이미지 파일의 핑퐁 번호와 과거의 안정적인 체크포인트 이미지 파일의 핑퐁 번호가 일치하지 않을 수 있다.
...
체크포인트 스케일을 싱글로 설정했을 경우, 더티 페이지의 비율에 따라 체크포인트 이미지 파일을 기록하는 방법이 다르다.
...
- **풀 라이트**
  ...
  2. 불안정한 체크포인트 이미지 파일에 더티 페이지가 포함된 체크포인트 이미지 파일의 내용 전부를 기록한다. 
...
**체크포인트 스케일 싱글에서 체크포인트 이미지 파일 복구** -1안

 손상된 체크포인트 이미지 파일을 복구하는 과정은 다음과 같다.

1. 장애가 발생한 서버를 구동한다. 이 때 META 구동 단계에서 자동으로 더블 라이트 리커버리(Double Write Recovery)가 수행되어 페이지가 복구된다.
2. 체크포인트를 수행하여 복구된 페이지를 체크포인트 이미지 파일에 기록한다.

더블 라이트 리커버리는 손상되었을 가능성이 있는 더티 페이지를 더블 라이트 이미지 파일을 이용하여 자동으로 복구하는 동작이다.
...
하지만 더블 라이트 리커버리는 더티 페이지 복구에만 국한된다. 손상된 체크포인트 이미지 파일을 완전히 복구하려면, 더블 라이트 리커버리 후 체크포인트를 발생시켜 복구된 페이지를 체크포인트 이미지 파일에 기록해야 한다.
...
**체크포인트 스케일 싱글에서 체크포인트 이미지 파일 복구** -2안
...
- 안정적인 체크포인트 이미지 파일 복구

  더블 라이트 리커버리는 더티 페이지 복구에만 국한된다. 손상된 체크포인트 이미지 파일을 완전히 복구하려면, 더블 라이트 리커버리 후 체크포인트를 발생시켜 복구된 페이지를 체크포인트 이미지 파일에 기록해야 한다. 즉, 손상된 체크포인트 이미지 파일을 복구하는 과정은 다음과 같다.
