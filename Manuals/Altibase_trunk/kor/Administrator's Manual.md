...
Altibase ACI 애플리케이션 작성을 위한 라이브러리이다. 자세한 내용은 *[Altibase C Interface Manual](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Altibase%20C%20Interface%20Manual.md)*을 참조한다.
...
Altibase는 각 메모리 테이블스페이스의 체크포인트 이미지를 SPLIT EACH 절에 지정한 크기만큼 나누어 체크포인트 이미지 파일에 기록한다. 즉, 체크포인트 이미지의 크기가 지정한 크기보다 커지면 체크포인트 이미지를 여러 파일로 분할한다. 분할된 체크포인트 이미지 파일은 생성된 순서대로 0부터 1씩 증가하는 파일 번호를 부여받는다. 체크포인트 이미지 파일 이름에 관한 좀 더 자세한 설명은 [체크포인트 이미지 파일 이름 규칙](#checkpointimagefilenaming)을 참고하라.
...
체크포인트(checkpoint)는 주기적으로 메모리 데이터베이스의 내용을 백업 데이터 파일에 저장하는 것을 말한다. 체크포인트 수행은 시스템 장애로부터 데이터베이스 복구에 걸리는 시간을 줄이는 데 그 목적이 있다.
...
- **풀 라이트**
  ...
  더티 페이지가 체크포인트 이미지 파일의 50%를 초과할 때 기록하는 방식이다.
...
더블 라이트 리커버리는 더블 라이트 동작 중 서버가 비정상 종료된 경우, 안정적인 체크포인트 이미지 파일에 기록 중이던 더티 페이지를 메모리 공간의 페이지에 복구하는 기능으로 Altibase 서버를 구동 할 때 META 단계에서 자동으로 수행된다. 이후 체크포인트가 실행되면 복구된 더티 페이지가 체크포인트 이미지 파일에 기록되어 데이터베이스의 영속성을 보장한다.
...
| 전체 데이터베이스의 온라인 백업   | SQL 문 이용 ALTER DATABASE BACKUP DATABASE TO ‘BACKUP_DIR’;  | 시스템 전체 테이블스페이스의 데이터 파일과 로그 앵커파일 | 1\> 유닉스 명령어 cp 이용 <br>2\> ALTER DATABASE RECOVER DATABASE; | O                |
| 특정 테이블스페이스의 온라인 백업 | 1\> SQL 문 이용<br>ALTER DATABASE BACKUP TABLESPACE 테이블스페이스이름 TO ‘backup_dir’; <br>또는 ALTER TABLESPACE 테이블스페이스 이름 BEGIN BACKUP; <br>2\> 유닉스 명령어 cp 사용<br>cp \<원본 파일\> \<backup_dir\><br>3\> ALTER TABLESPACE 테이블스페이스 이름 END BACKUP; | 테이블스페이스의 모든 데이터 파일                        | 1\> 유닉스 명령어 cp 이용.<br>2\> ALTER DATABASE RECOVER DATABASE; | O                |
| 오프라인 백업                     | 1\>데이터베이스 종료<br>2\>유닉스 명령어 cp 이용             | 전체 데이터베이스                                        | 유닉스 명령어 cp 이용                                        | X                |
...
이후 데이터베이스에서 **현재 시점의 로그 앵커**를 조회하여 복사된 체크포인트 이미지 파일의 핑퐁 번호를 일치시켜야 한다. 이때, 체크포인트 스케일에 따라 로그 앵커에서 확인해야 할 항목이 다르다는 점을 주의한다.

체크포인트 스케일은 [LOGANCHOR HEADER]의 Checkpoint Scale 항목의 값으로 알 수 있다. 
