...
‘DEFAULT’ 절을 사용해서 정의할 수 있으며, 생략 가능하다.

기본파티션이 있는 파티션드 테이블에 기본테이블을 삭제할 수 없고,
기본파티션이 없는 파티션드 테이블에 기본테이블을 추가할 수 없기 때문에
범위 파티셔닝 테이블을 생성시 주의가 필요하다.
...
파티션 추가는 한개의 파티션을 추가하는 기능이다. 기본파티션이 생략된 범위 파티션드 테이블에서만 사용할 수 있다.
