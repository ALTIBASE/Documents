...
IF NOT EXISTS 절을 명시하면 같은 이름의 디렉토리 객체가 존재하지 않는 경우에만 생성한다. 같은 이름의 디렉토리 객체가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 디렉토리 객체에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 인덱스가 존재하지 않는 경우에만 생성한다. 같은 이름의 인덱스가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 인덱스에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 큐가 존재하지 않는 경우에만 생성한다. 같은 이름의 큐가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 큐에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 이중화 객체가 존재하지 않는 경우에만 생성한다. 같은 이름의 이중화 객체가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 이중화 객체에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 시퀀스가 존재하지 않는 경우에만 생성한다. 같은 이름의 시퀀스가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 시퀀스에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 시노님이 존재하지 않는 경우에만 생성한다. 같은 이름의 시노님이 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 시노님에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 테이블이 존재하지 않는 경우에만 생성한다. 같은 이름의 테이블이 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 테이블에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 테이블스페이스가 존재하지 않는 경우에만 생성한다. 같은 이름의 테이블스페이스가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 테이블스페이스에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 트리거가 존재하지 않는 경우에만 생성한다. 같은 이름의 트리거가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 트리거에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 사용자가 존재하지 않는 경우에만 생성한다. 같은 이름의 사용자가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 사용자에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 뷰가 존재하지 않는 경우에만 생성한다. 같은 이름의 뷰가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 뷰에는 아무 영향을 주지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 materialized view가 존재하지 않는 경우에만 생성한다. 같은 이름의 materialized view가 존재하는 경우, CREATE 문은 이름 중복 오류 없이 실행되며 기존의 materialized view에는 아무 영향을 주지 않는다.
