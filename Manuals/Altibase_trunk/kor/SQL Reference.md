...
체크포인트 이미지 파일 관리 방식인 [체크포인트 스케일](https://github.com/ALTIBASE/Documents/blob/master/Manuals/Altibase_trunk/kor/Administrator's%20Manual.md#%EC%B2%B4%ED%81%AC%ED%8F%AC%EC%9D%B8%ED%8A%B8-%EC%8A%A4%EC%BC%80%EC%9D%BCcheckpoint-scale)을 변경한다. 이 절은 Altibase 서버 구동 순서 중 CONTROL 단계에서만 수행할 수 있다.

`CHECKPOINT SCALE PAIR`는 체크포인트 이미지 파일 관리 방식을 체크포인트 스케일 페어로 변경한다. 

`CHECKPOINT SCALE SINGLE`은 체크포인트 이미지 파일 관리 방식을 체크포인트 스케일 싱글로 변경한다.

체크포인트 스케일을 페어에서 싱글로 변경하면 체크포인트 이미지 한 쌍 중 불안정한 체크포인트 이미지 파일을 삭제하며, 싱글에서 페어로 변경하면 체크포인트 이미지 파일을 복사하여 한 쌍으로 만든다.
...
체크포인트 스케일을 지정하지 않으면  `CHECKPOINT SCALE PAIR`가 설정된다.
