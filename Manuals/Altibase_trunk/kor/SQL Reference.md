...
IF NOT EXISTS 절을 명시하면 같은 이름의 디렉토리 객체가 존재하지 않는 경우에만 생성한다. 같은 이름의 디렉토리 객체가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 디렉토리 객체는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 인덱스가 존재하지 않는 경우에만 생성한다. 같은 이름의 인덱스가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 인덱스는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 큐가 존재하지 않는 경우에만 생성한다. 같은 이름의 큐가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 큐는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 이중화 객체가 존재하지 않는 경우에만 생성한다. 같은 이름의 이중화 객체가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 이중화 객체는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 시퀀스가 존재하지 않는 경우에만 생성한다. 같은 이름의 시퀀스가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 시퀀스는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 시노님이 존재하지 않는 경우에만 생성한다. 같은 이름의 시노님이 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 시노님은 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 테이블이 존재하지 않는 경우에만 생성한다. 같은 이름의 테이블이 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 테이블은 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 테이블스페이스가 존재하지 않는 경우에만 생성한다. 같은 이름의 테이블스페이스가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 테이블스페이스는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 트리거가 존재하지 않는 경우에만 생성한다. 같은 이름의 트리거가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 트리거는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 사용자가 존재하지 않는 경우에만 생성한다. 같은 이름의 사용자가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 사용자는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 뷰가 존재하지 않는 경우에만 생성한다. 같은 이름의 뷰가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 뷰는 아무 영향을 받지 않는다.
...
IF NOT EXISTS 절을 명시하면 같은 이름의 materialized view가 존재하지 않는 경우에만 생성한다. 같은 이름의 materialized view가 존재하는 경우, CREATE 문은 오류 없이 실행되지만 기존의 materialized view는 아무 영향을 받지 않는다.
