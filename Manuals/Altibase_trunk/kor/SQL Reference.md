...
* TCP : TCP 통신방법을 사용.
* IB : 인피니밴드 통신방법을 사용. 인피니밴드를 사용할 경우에만 *ib_latency* 값을 설정할 수 있다. 인피니밴드를 사용하려면 IB_ENABLE 프로퍼티 값이 1이어야 한다.
...
데이터베이스의 체크포인트 스케일을 설정한다. 이 구문을 생략하면 체크포인트 스케일은 페어로 설정된다.
...
이중화 객체의 RECOVERY, OFFLINE, GROUPING, PARALLEL, GAPLESS, RECEIVE_ONLY, META_LOGGING 옵션을 지정하는 절이다. 자세한 설명은 *Replication Manual*을 참고한다.
...
| 문자 함수                       | 문자열 입력 값에 대한 작업을 수행하고 문자열이나 숫자 값을 반환한다. <br /><br />**문자열 반환 함수** CHR, CHOSUNG, CONCAT, DIGITS, INITCAP, LOWER, LPAD, LTRIM, NCHR, PKCS7PAD16, PKCS7UNPAD16, RANDOM_STRING, REGEXP_SUBSTR, REGEXP_REPLACE, REPLICATE, REPLACE2, REVERSE_STR, RPAD, RTRIM, STUFF, SUBSTRB(SUBSTR, SUBSTRING), TRANSLATE, TRIM, UPPER <br /><br />**숫자 값 반환 함수** ASCII, CHAR_LENGTH(CHARACTER_LENGTH,LENGTH), DIGEST,INSTR(POSITION, INSTRB), OCTET_LENGTH(LENGTHB), REGEXP_COUNT, REGEXP_INSTR, SIZEOF |
...
이 함수는 임의의 입력값을 CLOB 타입으로 변환한다.

##### 예제

```sql
iSQL>  select to_clob( 'test clob' ) from dual;
TO_CLOB( 'test clob' )
-----------------------------------------------------------------------------------
test clob
1 row selected.
...
이 함수는 임의의 입력값을 BLOB 타입으로 변환한다.

##### 예제

```sql
iSQL> create table tab_blob ( i1 blob );
Create success.
iSQL> insert into tab_blob values ( to_blob(1234) );
1 row inserted.
```
