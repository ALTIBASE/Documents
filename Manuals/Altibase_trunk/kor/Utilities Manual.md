...
다른 슬레이브 파드의 이중화는 시작되어 있는데 마스터 파드의 이중화 정보만 초기화된 경우, `aku -p start` 만 수행하여 다른 슬레이브 파드와의 연결을 복구 할 수 없다. 이 경우, 다른 파드로부터 데이터의 동기화를 수행한 다음 마스터 파드의 이중화를 시작하고 `aku -p start` 를 수행해야 한다. 자세한 절차는 아래의 조치를 참고 한다.
...
* **마스터 파드의 복구를 위한 조치**  (또는 마스터 파드의 수동 복구 절차)
  ...
  4. (슬레이브 파드 중 하나에서) 다른 파드에서 마스터 파드로의 SYNC를 수행한다.
     ...
     예를 들어, 마스터파드와 연결된 파드가 2개 인 경우, pod 1 또는 pod2 에서 아래의 명령어를 수행한다.
  ...
  6. (마스터 파드에서) `aku -p start` 명령을 수행한다.
