...
| AKU_REPLICATION_RESET_AT_END         |   1    | 슬레이브 파드(Slave Pod) 에서 aku -p end 명령 수행 시 RESET 명령으로 이중화 객체를 초기화할 것인지 설정한다.<br />1이면 이중화 객체를 초기화하며, 0이면 초기화하지 않는다. |
...
- ***스케일 업(Scale up)***
  ...
  > **슬레이브 파드를 처음 생성하거나 정상적으로 종료하고 다시 시작할 때 (AKU_REPLICATION_RESET_AT_END = 1, 기본 동작)**

  슬레이브 파드를 처음 생성하거나, 정상 종료된 슬레이브 파드를 다시 시작시키면서  `aku -p start`를 수행할 때의 aku 동작을 설명한다. 
