...
- ***스케일 업(Scale up)***
  ...
  2️⃣ *AKU_SERVER_COUNT*-1만큼 Altibase 이중화 객체를 생성한다. *pod_name*-1이 다시 시작된 파드라면, 같은 이름의 이중화 객체가 존재할 수 있으며 이 단계는 생략된다.
  ...
  파드가 비정상적으로 종료되거나 AKU_REPLICATION_RESET_AT_END 프로퍼티를 0으로 설정하고 종료하면, 이중화 정보가 초기화 되지 않는다. 이중화 정보를 초기화 하지 않으면 Altibase에 이전의 이중화 정보가 남아 있어, 파드를 다시 시작할 때 다른 노드로 부터 데이터를 동기화하는 작업(이중화 대상 테이블의 TRUNCATE, 이중화 SYNC 수행)을 수행하지 않는다. 다른 노드로부터 데이터 동기화는 하지 않지만, 자기 노드에서 다른 노드로의 데이터 동기화(FLUSH)는 수행한다. 아래는 이러한 경우에서 `aku -p start`를 수행할 때의 aku 동작을 설명한다.
  
  참고로, 이중화 정보가 초기화 되지 않으면 이중화 관련 메타 테이블에 이중화 재시작지점(XSN)이 -1이 아닌 값을 갖는데, 아래 주의사항 6을 참고한다.
...
### 3)임의적으로 마스터 파드의 Altibase 데이터베이스를 삭제하고 다시 생성한 경우

n개 이상의 파드 구성에서 마스터 파드의 이중화 정보가 초기화 된 경우, `aku -p start` 를 수행하면 아래의 오류 메시지를 확인하게 된다. 이 메시지는 마스터 파드에서 관리하는 이중화의 시작 정보가 없는 상태에서 슬레이브 파드가 시작되어 있는 경우, 마스터 파드의 장애(failure)로 판단하여 출력한다.

### 3) 마스터 파드에서 aku -p start 명령이 실패한 경우

아래의 경우, '**마스터 파드의 복구를 위한 조치**'를 참고하여 진행해야 한다.

2개 이상의 파드 구성에서 마스터 파드만 장애가 발생한 경우, aku -p start  수행시 아래의 오류 메시지를 확인한 경우, '마스터 파드의 복구를 위한 조치'를 참고한다.

* 마스터 파드의 Altibase 데이터베이스가 삭제된 경우
* 마스터 파드의 이중화 객체가 삭제된 경우
* n개 이상의 파드 구성에서 마스터 파드의 이중화 정보만 초기화 된 경우(슬레이브 파드는 시작되어 있는 상태)
* 마스터 파드에 연결이 끊어진 경우
...
### 3) 마스터 파드에서 관리하는 이중화의 시작 정보가 없는 상태로 슬레이브 파드가 시작되어 있는 경우

n개 이상의 파드 구성에서 마스터 파드의 이중화 시작정보

### 3) 마스터 파드에서 aku -p start 명령이 실패한 경우

 2개 이상의 파드 구성에서 마스터 파드의 이중화 정보만 초기화 된 경우, `aku -p start` 를 수행하면 아래의 오류 메시지를 확인하게 된다. 이 메시지는 마스터 파드에서 관리하는 이중화의 시작 정보가 없는 상태에서 슬레이브 파드가 시작되어 있는 경우, 마스터 파드의 장애(failure)로 판단하여 출력한다.

```
[ERROR] The master pod is detected to have failed. Check and perform a manual recovery.
```

이러한 경우, XSN을 조회해보면 아래와 같이 -1인것을 확인할 수 있다.
...
마스터 파드의 이중화 정보가 초기화된 경우, `aku -p start` 만 수행하여 다른 슬레이브 파드와의 연결을 복구 할 수 없다. 이 경우, 다른 파드로부터 데이터의 동기화를 수행한 다음 마스터 파드의 이중화를 시작하고 `aku -p start` 를 수행해야 한다. 자세한 절차는 아래의 조치를 참고 한다.

아래의 조치는 마스터 파드와 연결된 이중화 객체는 생성되어 있다는 전제로 수행한다. 만약 이중화 객체가 삭제되었다면, 이중화를 위한 테이블 및 이중화 객체를 생성한 다음 아래의 조치를 진행해야 한다.

* **마스터 파드의 복구를 위한 조치**  (또는 마스터 파드의 수동 복구 절차)

  1. (마스터 파드에서) 알티베이스 서버 프로퍼티를 아래와 같이 설정한다.

     - ADMIN_MODE=1
     - REPLICATION_DDL_ENABLE=1

  2. (마스터 파드에서) 데이터 SYNC를 수행하기 전에 테이블의 데이터를 truncate 명령어로 삭제한다.

  3. (마스터 파드에서) 알티베이스 서버 프로퍼티 REPLICATION_DDL_ENABLE을 다시 변경한다.

     - REPLICATION_DDL_ENABLE=0

  4. (슬레이브 파드 중 하나에서) 다른 파드에서 마스터 파드로의 SYNC를 수행한다.

     예를 들어, 마스터파드와 연결된 파드가 n개 이고 pod 1(AKU_REP_01) 또는 pod2(AKU_REP_02) 에서 아래의 명령어를 수행한다.

     ```sql
     ALTER REPLICATION replication_name SYNC;
     ```

  5. (마스터 파드에서) 4에서 명령어를 수행한 파드와 연결된 이중화 객체를 시작시킨다.

     ```sql
     ALTER REPLICATION replication_name START;
     ```

  6. (마스터 파드에서) aku -p start 수행한다.

* 마스터 파드의 장애(failure)

  aku에서 파드들은 순차적으로 스케일 업, 스케일 다운한다는 전제로 동작한다. 그런데 마스터 노드가 아래의 경우에 해당하고 슬레이브 파드가 시작 상태인 경우, 마스터 파드의 장애로 판단한다. 이런 경우, 반드시 '**마스터 파드의 복구를 위한 조치**'를 참고하여 진행해야 한다.

  * 임의적으로 마스터 파드의 Altibase 데이터베이스를 삭제하고 다시 생성한 경우 (컨테이너 정책 또는 configmap을 이용하여 데이터베이스를 삭제하고 재 생성하도록 스크립트를 작성한 경우)
  * 실수로 마스터 파드의 이중화 객체를 reset 한경우
  * 실수로 마스터 파드의 이중화 객체를 삭제한 경우
