...
### 3) 마스터 파드의 이중화 정보가 초기화 된 경우

 (1) n개 이상의 파드 구성에서 마스터 파드의 이중화 정보가 초기화 된 경우, `aku -p start` 를 수행하면 아래의 오류 메시지를 확인하게 된다. 이 메시지는 마스터 파드에서 관리하는 이중화의 시작 정보가 없는 상태에서 슬레이브 파드가 시작되어 있는 경우, 마스터 파드의 장애(failure)로 판단하여 출력한다.

(2) n개 이상의 파드 구성에서 마스터 파드의 이중화 정보가 초기화 된 경우, 다른 슬레이브 파드와의 재 연결을 위해서는 수동 복구 절차가 필요하다. 수동 복구 절차를 진행하지 않고 `aku -p start` 를 수행하는 경우, aku는 마스터 파드의 상태를 장애(failure)로 판단하고 아래의 오류 메시지를 출력한다.
...
* 마스터 파드의 이중화 정보가 초기화 되는 경우

  마스터 파드의 경우 `aku -p end` 를 수행하여도 이중화 정보가 초기화 되지 않는데, 아래의 경우 예외적으로 이중화 정보가 초기화 된다. 이런 경우는 반드시 '**마스터 파드의 복구를 위한 조치**'를 참고하여 진행해야 한다.
