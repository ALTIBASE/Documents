...
-   **동기화 (Synchronization):**  
    ...
    원격 서버에서 데이터 충돌이 발생하면 SYNC 작업이 실패할 수 있다.
...
-   SYNC  
    ...
    레코드가 있으면, SYNC 작업이 실패할 수 있다. REPLICATION_SYNC_TUPLE_COUNT
    프로퍼티 값을 1로 설정하면 실패를 방지할 수 있다.
...
-   SYNC ONLY  
    ...
    같은 프라이머리 키 값을 가지는 레코드가 있으면, SYNC 작업이 실패할 수 있다.
    REPLICATION_SYNC_TUPLE_COUNT 프로퍼티 값을 1로 설정하면 실패를 방지할 수
    있다.
...
-   STOP  
    ...
    삭제한 후 SYNC를 다시 수행해야 실패를 방지할 수 있다.
