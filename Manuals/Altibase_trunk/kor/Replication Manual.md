...
만약 위와 같은 이중화 객체에 속한 대상에 DDL 문을 수행하려면, [부록 B의 기본 이중화 대상 DDL 문 수행 방법](#기본-이중화-대상-DDL-문-수행-방법)을 참고한다.
...
아래의 예제는 삼중화 환경에서 *rep1*의 이중화 대상 테이블 *T1*에 NOT NULL 제약조건을 가진 칼럼 *c1*을 추가한다.
...
이 절에서는 이중화 대상에 DDL 문을 수행할 때 어쩌구 저쩌구.
...
> [!caution]
>
> SQL 반영 모드는 DDL 문 수행 작업 중 일시적으로만 사용하고, **사용 후 즉시 비활성화**해야 한다. SQL 반영 모드로 동작하는 이중화 대상의 복제 속도는 XLog를 반영하는 것보다 현저히 느리며 데이터 불일치가 발생할 수 있다. ??? 느리기때문에 트랜잭션이 꼬여서 데이터 불일치가 발생할 수 있다??
