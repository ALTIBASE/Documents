...
- [Replication Manual](#replication-manual)
  ...
  - [3.이중화 사용](#3%EC%9D%B4%EC%A4%91%ED%99%94-%EC%82%AC%EC%9A%A9)
    ...
    - [고정 IP를 지정하여 이중화](#고정-ip를-지정하여-이중화)
...
이중화 통신 시 사용할 고정 IP 주소를 지정하는 기능을 제공한다. IP 주소가 변경되는 다중 IP 네트워크 환경에서 고정 IP 주소를 이중화 통신 전용으로 설정할 수 있다.

#### 설명

다중 IP 네트워크 환경에서 특정 IP 주소를 이중화 통신 전용으로 구성하여 서비스 통신 라인과 이중화 통신 라인을 완전히 분리할 수 있다. 또한, Virtual IP 주소 사용 등의 이유로 유동적으로 변경되는 네트워크 환경에서 고정 IP 주소를 지정하는 기능으로 이중화를 보다 안정적으로 운영할 수 있다.

이중화 IP 주소 지정은 Altibase 서버 프로퍼티로 설정하며, 이중화 객체 단위로 설정할 수 없다.

**REPLICATION_IP 프로퍼티**

REPLICATION_IP 프로퍼티에 고정 IP를 입력하고 Altibase 서버를 구동하면, 이중화 관리자 초기화 과정에서 이중화 통신에 사용할 IP를 설정한다.

##### 제약사항
- TCP만 지원하며, IB(Infini Band) 통신은 지원하지 않는다.
- REPLICATION_IP 프로퍼티 값에 호스트 이름을 설정할 수 없다.
- 양쪽 이중화 대상 서버에 REPLICATION_IP 프로퍼티를 설정하는 경우, 통신 프로토콜 버전이 같아야한다. 예를들어 지역서버에는 IPv6, 원격서버에는 IPv4로 설정하면 네트워크 오류로 이중화에 실패한다.
