...
Altibase 7.2 JDBC는 Type 4 pure Java JDBC  드라이버로서, JDBC 4.2 API를 부분적으로 지원한다. Altibase 7.2 JDBC 드라이버를 사용하기 위한 자바 최소 버전은 JRE 1.8이며 지원하는 JDBC 4.2 API는 [JDBC 4.2 API References](#6jdbc-42-api-references)에서 확인할 수 있다.

#### JDBC 드라이버 종류

Altibase 7.2는 3가지 종류의 JDBC 드라이버를 제공한다.

###### Altibase.jar

Altibase 7.2의 기본 JDBC 드라이버이다.

###### Altibase7_2.jar

기본 JDBC 드라이버와 동일하지만 드라이버 클래스 이름이 다르다. 이 JDBC 드라이버는 하나의 애플리케이션에서 여러 버전의 Altibase 서버에 접속할 때 Altibase 7.2 JDBC 드라이버를 구별하는 용도로 사용하며 드라이버 클래스 이름은 Altibase7_2.jdbc.driver.AltibaseDriver이다.

Altibase 서버 버전에 상관없이 기본 JDBC 드라이버 파일 이름과 드라이버 클래스 이름이 같아서 Altibase.jar로 Altibase 서버 버전을 구분할 수 없기 때문에 Altibase 7.2 서버 접속을 위한 추가적인 JDBC 드라이버 Altibase7_2.jar를 제공한다.

다음은 서로 다른 버전의 Altibase 서버에 동시 접속하는 자바 코드 예제이다. 아래 예에서 Altibase.jar는 Altibase 7.1 JDBC 드라이버라고 가정한다.

~~~java
// Altibase 7.2 JDBC 드라이버를 구별하는 Altibase7_2.jar의 JDBC 드라이버 클래스
Class.forName("Altibase7_2.jdbc.driver.AltibaseDriver");
...
JDBC 로깅 기능을 포함한 드라이버이다. 이 드라이버의 사용 방법은 [JDBC 로깅](#jdbc-로깅)에서 확인할 수 있다. 

###### Altibase 7.2 JDBC 드라이버 종류와 특성

다음은 Altibase 7.2에서 제공하는 JDBC 드라이버 종류와 특성을 정리한 표이다.

| JDBC 드라이버 종류 | 용도                                                         |   JDBC API    | 최소 JRE 버전 | 드라이버 클래스 이름                   |
| :----------------- | :----------------------------------------------------------- | :-----------: | :-----------: | :------------------------------------- |
| Altibase.jar       | 기본 JDBC 드라이버                                           | 4.2 부분 지원 |      1.8      | Altibase.jdbc.driver.AltibaseDriver    |
| Altibase7_2.jar    | 여러 버전의 Altibase 서버 접속하는 애플리케이션에서 Altibase 7.2를 구별하기 위한 JDBC 드라이버 | 4.2 부분 지원 |      1.8      | Altibase7_2.jdbc.driver.AltibaseDriver |
| Altibase_t.jar     | JDBC 로깅 기능을 포함한 드라이버                             | 4.2 부분 지원 |      1.8      | Altibase.jdbc.driver.AltibaseDriver    |
...
Altibase JDBC를 사용하려면 Altibase.jar 파일을 CLASSPATH 환경 변수에 추가해야 한다.
...
Altibase 7.2 JDBC 드라이버에서 지원하는 기능과 지원하지 않는 기능을 보여주는 표이다.
...
| 인터페이스명                   | JDBC API 버전 | 지원여부 | 설명                                                         | 예외 처리 |
|------------------------------------------------------------|:--------:|:--------:|--------------------------------------------------------------------------|------------------------------------------------------|
| setPoolable(boolean poolable)                              | 4.0      |    O     | 알티베이스 JDBC에서 Statement Pool은 지원하지 않고 플래그 설정만 가능   |                                                      |
| isPoolable()                                               | 4.0      |    O     |                                                                          |                                                      |
...
| 인터페이스명                                                 | JDBC API 버전 | 지원여부 | 설명                                           | 예외 처리 |
|----------------------------------------------------------------|:--------:|:--------:|--------------------------------------------------------------------------|------------------------------------------------------|
| addStatementEventListener(StatementEventListener listener)     | 4.0      |    X     | Statement Pool을 미지원하므로 동작이 무시된다.    |                                                      |
| removeStatementEventListener(StatementEventListener listener)  | 4.0      |    X     | Statement Pool을 미지원하므로 동작이 무시된다.    |                                                      |
...
| updateNClob(int columnIndex, Reader reader)                                | 4.0      |    X     | NCLOB 미지원        | SQLFeatureNotSupported 예외 발생                      |
| updateNClob(int columnIndex, Reader reader, long length)                   | 4.0      |    X     | NCLOB 미지원       | SQLFeatureNotSupported 예외 발생                      |
| updateNClob(String columnLabel, Reader reader)                             | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNClob(String columnLabel, Reader reader, long length)                | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNClob(int columnIndex, NClob nClob)                                  | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNClob(String columnLabel, NClob nClob)                               | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| getNClob(int columnIndex)                                                  | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| getNClob(String columnLabel)                                               | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
...
| getNCharacterStream(int columnIndex)                                       | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| getNCharacterStream(String columnLabel)                                    | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNCharacterStream(int columnIndex, Reader x)                          | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNCharacterStream(int columnIndex, Reader x, long length)             | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNCharacterStream(String columnLabel, Reader reader)                  | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
| updateNCharacterStream(String columnLabel, Reader reader, long length)     | 4.0      |    X     | NCLOB 미지원                | SQLFeatureNotSupported 예외 발생                      |
...
| 인인터페이스명        | JDBC API 버전 | 지원여부 | 설명 | 예외 처리 |
|-----------------------------------------------|:--------:|:--------:|----------------------------------------|--------------------------------|
| getName()                                     | 4.2      |    O     |                                        |                                |
| getVendor()                                   | 4.2      |    O     |                                        |                                |
