...
DELETE ON은 큐 테이블에 DELETE 문 사용을 허용한다. DELETE OFF는 큐 테이블에 DELETE 구문을 허용하지 않는다. 이 경우 DELETE 문을 허용한 경우보다 DEQUEUE 병렬 수행 성능이 향상된다.
...
사용자 정의 칼럼을 지정한다. CREATE TABLE 의 column_definition을 참고하며, 해당 구문에서 column_constraint, crypt_clause, timestamp는 지원하지 않는다.
...
- INSERT  
  해당 테이블에 데이터를 삽입하는 INSERT 구문 수행 시 트리거 동작이 유발된다. LOB 칼럼이 있는 테이블의 경우 'BEFORE INSERT ... FOR EACH ROW' 구문으로 트리거를 생성할 수는 있으나, 트리거의 동작을 유발하는 DML문이 실행될 때 오류가 발생한다.
- UPDATE  
  해당 테이블의 데이터를 변경하는 UPDATE 구문 수행 시 트리거 동작이 유발된다.
  UPDATE 트리거 이벤트에 OF 절을 사용할 경우 OF 절에 명시된 컬럼이 변경될 경우에만 트리거를 동작시킨다. LOB 칼럼이 있는 테이블의 경우 'BEFORE UPDATE ... FOR EACH ROW' 구문으로 트리거를 생성할 수는 있으나, 트리거의 동작을 유발하는 DML문이 실행될 때 오류가 발생한다.
