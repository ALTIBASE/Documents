...
현재 Altibase는 테이블을 스캔하는 병렬 질의와 실행 계획에 HASH, SORT, GRAG 노드가 포함되는 병렬 질의만을 지원한다. 단, 이러한 노드의 경우에는 각 노드당 병렬 작업 쓰레드가 한 개씩만 생성된다.
...
- conn_type
  ...
  원격 서버와 통신 방법 ( TCP / IB )을 나타낸다.

- ib_latency

  rsocket의RDMA_LATENCY 옵션 값, conn_type이 IB인 경우에만 입력 가능하다.
...
이는 테이블의 데이터를 변경시키는 이벤트로 트리거의 동작을 유발시킨다. 단 데이터베이스의 무결성을 지키기 위해 이중화 수신자에 의해 적용되는 테이블 데이터의 변경은 트리거 이벤트로 처리되지 않는다 (즉, 트리거 동작을 유발시키지 않는다). 한 개의 트리거에 여러 개의 트리거 이벤트를 지정할 수 있다. *trigger_event*로 다음의 세 가지 유형의 DML문을 지정할 수 있다.
...
- {OLD\|OLD ROW\|OLD ROW AS\|OLD AS} alias_name  
  변경되기 이전의 로우(row)를 의미한다. 이는 WHEN 절 또는 trigger_action의 psm_body 내에서 참조될 수 있다. 트리거 이벤트가 INSERT문일 때는 이전의 값이 없기 때문에 NULL값을 갖는다.
...
- *unpivot_clause*
  ...
  unpivot_clause의 pivot_for_clause 와 pivot_in_clause에 사용되는 column 의 개수는 같아야 한다. 또한 alias 의 개수도 같아야 한다. 
  
  unpivot_clause에서 "( column, column )"으로 컬럼 2개를 사용하면, pivot_for_clause에서도 "for ( column, column )", pivot_in_clause 에서도 "in ( ( column, column ) as ( alias, alias ), ... )" 으로 동일하게 2개씩 사용한다.
