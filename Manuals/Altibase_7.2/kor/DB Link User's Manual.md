...
원격 서버 ~~캐릭터셋~~의 문자형 데이터 타입 중 CHAR와 VARCHAR의 단위와 크기를 고려하여 설정~~할 수 있다.~~한다.

알티베이스의 ~~문자형 데이터 타입 중~~ CHAR와 VARCHAR 데이터 타입의 길이는 고정형 바이트 단위이지만, 타 DBMS에서 바이트(BYTE) 또는 캐릭터(CHAR) 단위로 선택 가능한 경우가 있다. (예. Oracle의 NLS_LENGTH_SEMANTICS )

AltiLinker는 데이터 손실을 막기 위해 원격 서버의 CHAR와 VARCHAR 데이터 타입의 단위를 보수적으로 캐릭터(CHAR) 단위로 가정하고 지역 서버의 캐릭터셋에 맞춰 길이를 변환한다. 예를 들어 지역 서버의 캐릭터셋이 MS949이고 원격 서버의 CHAR, VARCHAR 데이터 타입의 길이(size)가 10으로 선언되어 있다면, MS949에서 한 글자를 2바이트로 표현하므로 10*2=20바이트로 길이를 변환한다.


 **0으로 설정 ~~해야~~ 하는 경우 (기본값)**

아래 두 조건은 *(은? 두 조건을 모두 만족해야하나요? 하나라도 만족하면 되나요?)*만족하는 경우 0으로 설정한다. 

- 원격 서버 ~~캐릭터셋~~의 문자형 데이터 타입 중 CHAR와 VARCHAR의 길이 단위가 캐릭터(CHAR) 인 경우

  - Oracle 경우, VARCHAR2(10 CHAR)

- 지역 서버 캐릭터셋과 원격 서버 캐릭터셋이 같거나, ~~문자형 데이터 타입 중~~ CHAR와 VARCHAR의 한 글자 크기가 같은 경우
...
*주석 추가했습니다. 필요 시 더 추가해주면 좋을 것 같습니다.*
...
- 원격 서버 ~~캐릭터셋~~의 문자형 데이터 타입 중 CHAR와 VARCHAR의 한 글자 크기가 1바이트인 경우 
- ~~지역 서버와 원격 서버의 CHAR와 VARCHAR의 길이 단위가 바이트(BYTE)인 경우~~ *(위와 동일한 설명 아닌가요? 삭제해도 되지 않을까요?)*

*아래 설명은 어떤가요?*

- *원격 서버의 캐릭터셋이 한 글자 당 1바이트이거나 (예, ASCII)*
- *원격 서버의 캐릭터셋 설정과 무관하게 CHAR, VARCHAR 데이터 타입의 단위를 BYTE로 생성한 경우* 
- *원격 서버의 데이터베이스가 알티베이스인 경우*

*(아래 표를 제공하는 의도는 무엇인가요? 제가 이해를 잘 못하겠는...;;;)*
...
예제 *주석 추가했습니다. 필요 시 더 추가해주면 좋을 것 같습니다.*
```sql
...
- 원격 서버 ~~캐릭터셋~~의 문자형 데이터 타입 중 CHAR와 VARCHAR의 단위가 캐릭터(CHAR), 길이 1이 2바이트 인 경우
...
예제) 지역 캐릭터셋 MS949, 원격 타사 DBMS 캐릭터셋 MS949로 character 단위의 컬럼 생성

-> *지역 서버와 원격 서버의 캐릭터셋이 MS949이고 CHAR 데이터 타입 단위를 CHAR로 생성*

*지역 서버, 원격 서버 구분해서 주석도 추가해주면 좋겠습니다.*
