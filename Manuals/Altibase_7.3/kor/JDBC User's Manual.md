...
| 필수 여부 | No                                                           |
| 설정 범위 | 세션                                                         |
| 설명      | TCP 소켓 옵션인 SO_LINGER의 활성화 여부를 설정한다.<br>- true: SO_LINGER 값을 0으로 설정한다. 소켓을 닫는 즉시 연결이 종료되고 남아있는 데이터는 전송되지 않는다.<br>- false: SO_LINGER을 비활성화한다. 소켓은 바로 닫히지만, 소켓 버퍼에 남아있는 데이터가 있다면 커널은 일정 시간 동안 데이터를 보내려고 시도한다.<br/>이 연결 속성은 Altibase JDBC 드라이버 7.3.0.0.7 이상부터 지원한다. |
