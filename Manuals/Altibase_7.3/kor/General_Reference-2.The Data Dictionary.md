...
- SYNC: 2
  ...
  지역 서버의 이중화 대상 테이블의 모든 데이터를 원격 서버의 대응하는 테이블의 데이터와 일치시키기 위해 수행하는 작업. 원격 서버로의 데이터 동기화 완료 후 현재 로그부터 이중화가 시작된다.
...
- RECOVERY from Replication : 6
  ...
  Remote Site에 손실된 트랜잭션이 변경한 로그와 대응되는 트랜잭션 로그가 있다면, 이 트랜잭션 로그를 이용하여 Local에서 손실된 데이터를 복구해 주는 작업.

- OFFLINE: 7

  설정된 오프라인 경로를 이용하여 이중화를 수행한다. 오프라인 이중화는 일회성 작업으로써, 미전송된 로그를 모두 반영한 후 바로 종료된다.

- PARALLEL: 8

  EAGER 모드에서 이중화 수행시 여러개의 송신 쓰레드를 사용하는 것을 일컫는다.
...
- 2: RETRY
  ...
  원격 서버와 연결에 장애가 발생하여 재시도를 하는 단계.

- 3: FAILBACK NORMAL

  증분 동기화를 완료 하거나 건너뛴 후, Eager 모드 이중화가 정상적으로 시작되기 전에, 장애 시간 동안 서버 B에서 서버 A로 전송하지 못했던 트랜잭션 로그에 대한 데이터를 동기화한다. 동기화 시에는 Lazy로 전환되었다가 동기화가 완료되면 EAGER로 다시 전환된다.

- 4: FAILBACK MASTER

  상대편 노드의 장애를 감지한 시점이 상대편보다 더 늦을 경우 MASTER 노드가 된다. SLAVE 노드로 SLAVE 가 요청한 데이터를 전송해 준다.

- 5: FAILBACK SLAVE

  상대편 노드의 장애를 감지한 시점이 상대편보다 더 빠를 경우 SLAVE 노드가 된다. 상대편으로 전송하지 않았을 것 같은 (즉, MASTER 와 다를 수 있는) 데이터를 결정하고, MASTER로부터 해당 데이터를 가져와서 동기화를 수행한다.
...
- 7: FAILBACK EAGER
  ...
  FAILBACK 과정 중 생긴 이중화 갭을 없애기 위해 EAGER 모드로 이중화를 수행한다.
