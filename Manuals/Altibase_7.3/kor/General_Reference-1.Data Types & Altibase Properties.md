...
CLOB은 문자형 대용량 데이타를 저장하기 위한 문자형 데이타 타입으로, 4GB-1byte 크기까지 저장 가능하다.
...
로그 파일의 크기를 지정하는 프로퍼티로, 지정된 크기에 도달하면 새로운 로그 파일에 기록한다. 이 프로퍼티는 데이터베이스 생성 시에 설정할 수 있으며, 생성 후에는 변경할 수
