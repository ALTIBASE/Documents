...
Altibase dataCompJ는 두 이종 데이터베이스간 데이터 불일치를 해소하여 데이터 일관성을 보장하기 위해 설계된 도구이다. dataCompj에서 비교를 하는 메인 대상 데이터는 Altibase에서 제공하는 Altibase Adapter for Oracle(oraAdapter) 또는 JDBC Adapter를 사용하여 Altibase에서 다른 이종 데이터베이스로 복제된다. dataCompJ는 Altibase와 다른 이종 데이터베이스를 테이블 단위로 비교하고 데이터 불일치가 있는 경우 이에 관한 정보를 생성한다. Altibase dataCompJ의 올바른 사용은 뛰어난 기술적 성능을 통해 효과적인 데이터베이스 관리의 초석이 될 것이다.  

### 1.1 하드웨어 요구사항

- 메인 메모리: 최소 512MB, 4GB 이상 권장
- 디스크: 설치를 위해 최소 500MB 이상의 여유 공간 필요
...
dataCompJ는 순수 Java 기반 클라이언트 애플리케이션으로, 클라이언트의 하드웨어나 OS보다 JAVA Runtime Environment (JRE)에 의존한다.
