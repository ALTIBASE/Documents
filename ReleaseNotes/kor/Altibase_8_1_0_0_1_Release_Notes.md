...
| AIX 7.2<br/>AIX 7.1<br/>AIX 6.1 TL9                       |       ●       |          ●          |                         |
| **Linux x86-64**                                          |               |                     |                         |
...
* **서버 구동시 메모리 인덱스 빌드 과정의 성능 개선** (BUG-51301)

* **서버 종료 및 메모리 인덱스 삭제시 성능 개선**(BUG-51787)

* **로그 파일 준비 쓰레드(Log File Prepare Thread)의 성능 개선**(BUG-50654)

  * LOG_FILE_SIZE 의 기본값이 10MB에서 100MB로 변경되었습니다.
  * LOG_CREATE_METHOD 의 기본값 변경

* **LOB 대량 삽입 작업의 성능 개선** (BUG-48211)

* **메모리 데이터베이스의 구동시 페이지 캐시 개선**(BUG-49872)
